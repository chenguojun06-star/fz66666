...
class DailyBriefOrchestratorTest {
    ...
    @Mock
    private DailyBriefDecisionOrchestrator dailyBriefDecisionOrchestrator;
    ...
    void setUp() {
        ...
        when(dailyBriefDecisionOrchestrator.buildDecisionCards(any(), anyLong(), anyLong(), anyLong(), anyLong(), any()))
            .thenReturn(List.of());
    }
    ...
    void getBrief_yesterdayWarehousingCountMatchesMock() {
        Map<String, Object> brief = orchestrator.getBrief();
        assertThat(((Number) brief.get("yesterdayWarehousingCount")).intValue()).isEqualTo(3);
    }

    @Test
    void getBrief_overdueCountMatchesMock() {
        Map<String, Object> brief = orchestrator.getBrief();
        assertThat(((Number) brief.get("overdueOrderCount")).intValue()).isEqualTo(2);
    }

    @Test
    void getBrief_todayScanCountMatchesMock() {
        Map<String, Object> brief = orchestrator.getBrief();
        assertThat(((Number) brief.get("todayScanCount")).intValue()).isEqualTo(50);
    }
}
