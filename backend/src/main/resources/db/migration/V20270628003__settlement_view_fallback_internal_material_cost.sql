-- 修复 v_finished_product_settlement 视图：内部工厂订单兜底读取 t_production_order.material_cost
-- 背景：视图原 material_cost 只统计 t_material_purchase（外部工厂物料采购），
--       内部工厂走 t_material_pickup_record 汇总到 t_production_order.material_cost，
--       导致内部工厂订单在「工资结算 > 内部工厂订单」Tab 面辅料费用恒为 0。
-- 修复：COALESCE(mat.total_material_cost, po.material_cost, 0)
--       外部工厂有 t_material_purchase 记录 → 优先用 mat.total_material_cost
--       内部工厂无 t_material_purchase 记录 → 兜底用 po.material_cost（由 OrderCostSummaryHelper 汇总）
