...
BEGIN
    ...
    THEN
        CREATE INDEX idx_outbound_date
            ON t_material_outbound_log ((DATE(COALESCE(outbound_time, create_time))));
    END IF;
    ...
    THEN
        CREATE INDEX idx_create_date
            ON t_material_outbound_log ((DATE(create_time)));
    END IF;
END //
CALL add_mat_outbound_date_idx() //
DROP PROCEDURE add_mat_outbound_date_idx;
...
BEGIN
    ...
    THEN
        CREATE INDEX idx_create_date
            ON t_production_order ((DATE(create_time)));
    END IF;
END //
CALL add_order_create_date_idx() //
DROP PROCEDURE add_order_create_date_idx;

-- 3. 兼容性检查：MySQL 8.0.13+ 才支持函数索引
-- 如果是低版本 MySQL，函数索引语句会报错，请确认云端 MySQL 版本 >= 8.0.13
