...
DROP PROCEDURE IF EXISTS `__mig_V20260223__unit_price_audit_and_pattern_version`;
DELIMITER $$
CREATE PROCEDURE `__mig_V20260223__unit_price_audit_and_pattern_version`()
BEGIN
    ...
    CREATE INDEX `idx_style_attachment_biz_type` ON `t_style_attachment` (`biz_type`);
    CREATE INDEX `idx_style_attachment_status`   ON `t_style_attachment` (`status`);
...
END$$
DELIMITER ;
CALL `__mig_V20260223__unit_price_audit_and_pattern_version`();
DROP PROCEDURE IF EXISTS `__mig_V20260223__unit_price_audit_and_pattern_version`;
