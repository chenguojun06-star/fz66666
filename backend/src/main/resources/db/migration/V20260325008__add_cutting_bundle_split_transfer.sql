ALTER TABLE `t_cutting_bundle`
    ADD COLUMN `root_bundle_id` VARCHAR(64) NULL COMMENT '主菲号ID' AFTER `id`,
    ADD COLUMN `parent_bundle_id` VARCHAR(64) NULL COMMENT '父菲号ID' AFTER `root_bundle_id`,
    ADD COLUMN `source_bundle_id` VARCHAR(64) NULL COMMENT '拆分来源菲号ID' AFTER `parent_bundle_id`,
