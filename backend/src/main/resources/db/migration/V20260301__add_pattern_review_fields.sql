...
DROP PROCEDURE IF EXISTS `__mig_V20260301__add_pattern_review_fields`;
DELIMITER $$
CREATE PROCEDURE `__mig_V20260301__add_pattern_review_fields`()
BEGIN
    ...
    CREATE INDEX `idx_pp_review_status` ON `t_pattern_production` (`review_status`);
    CREATE INDEX `idx_pp_review_by_id` ON `t_pattern_production` (`review_by_id`);

END$$
DELIMITER ;
CALL `__mig_V20260301__add_pattern_review_fields`();
DROP PROCEDURE IF EXISTS `__mig_V20260301__add_pattern_review_fields`;
