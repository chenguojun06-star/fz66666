...
DROP PROCEDURE IF EXISTS `__mig_V20260224__add_data_import_permission`;
DELIMITER $$
CREATE PROCEDURE `__mig_V20260224__add_data_import_permission`()
BEGIN
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION BEGIN END;
    -- 添加数据导入菜单权限
    INSERT IGNORE INTO t_permission (permission_code, permission_name, permission_type, description, create_time, update_time)
...
END$$
DELIMITER ;
CALL `__mig_V20260224__add_data_import_permission`();
DROP PROCEDURE IF EXISTS `__mig_V20260224__add_data_import_permission`;
