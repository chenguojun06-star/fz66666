...
DROP PROCEDURE IF EXISTS `__mig_V20260223b__remaining_tables_and_operator_fields`;
DELIMITER $$
CREATE PROCEDURE `__mig_V20260223b__remaining_tables_and_operator_fields`()
BEGIN
    ...
    ALTER TABLE `t_material_purchase`
        ...
        COMMENT '最新入库单ID';
    CREATE INDEX `idx_mpu_inbound_record_id` ON `t_material_purchase` (`inbound_record_id`);
    ...
    CREATE INDEX `idx_mpu_creator_id` ON `t_material_purchase` (`creator_id`);
    ...
    ALTER TABLE `t_product_outstock`
        ...
        ADD COLUMN `creator_id`    VARCHAR(32)  DEFAULT NULL COMMENT '创建人ID',
        ADD COLUMN `creator_name`  VARCHAR(100) DEFAULT NULL COMMENT '创建人姓名';
    CREATE INDEX `idx_pos_operator_id` ON `t_product_outstock` (`operator_id`);
    CREATE INDEX `idx_pos_creator_id`  ON `t_product_outstock` (`creator_id`);
    ...
    CREATE INDEX `idx_cb_creator_id`  ON `t_cutting_bundle` (`creator_id`);
    CREATE INDEX `idx_cb_operator_id` ON `t_cutting_bundle` (`operator_id`);
    ...
    CREATE INDEX `idx_sq_creator_id` ON `t_style_quotation` (`creator_id`);
    CREATE INDEX `idx_sq_auditor_id` ON `t_style_quotation` (`auditor_id`);
    ...
    CREATE INDEX `idx_pse_auditor_id`   ON `t_payroll_settlement` (`auditor_id`);
    CREATE INDEX `idx_pse_confirmer_id` ON `t_payroll_settlement` (`confirmer_id`);
    ...
    ALTER TABLE `t_cutting_task`
        ...
        ADD COLUMN `updater_id`   VARCHAR(32)  DEFAULT NULL COMMENT '更新人ID',
        ADD COLUMN `updater_name` VARCHAR(100) DEFAULT NULL COMMENT '更新人姓名';
    CREATE INDEX `idx_ct_creator_id` ON `t_cutting_task` (`creator_id`);
    ...
    CREATE INDEX `idx_spc_creator_id`  ON `t_secondary_process` (`creator_id`);
    CREATE INDEX `idx_spc_assignee_id` ON `t_secondary_process` (`assignee_id`);
    ...
    CREATE INDEX `idx_pp_receiver_id`      ON `t_pattern_production` (`receiver_id`);
    CREATE INDEX `idx_pp_pattern_maker_id` ON `t_pattern_production` (`pattern_maker_id`);
    ...
    ALTER TABLE `t_shipment_reconciliation`
        ...
        ADD COLUMN `auditor_name`                 VARCHAR(100) DEFAULT NULL COMMENT '审核人姓名',
        ADD COLUMN `audit_time`                   DATETIME     DEFAULT NULL COMMENT '审核时间';
    CREATE INDEX `idx_shr_reconciliation_operator_id` ON `t_shipment_reconciliation` (`reconciliation_operator_id`);
    CREATE INDEX `idx_shr_auditor_id`                 ON `t_shipment_reconciliation` (`auditor_id`);
    ...
    CREATE INDEX `idx_po_created_by_id` ON `t_production_order` (`created_by_id`);
...
END$$
DELIMITER ;
CALL `__mig_V20260223b__remaining_tables_and_operator_fields`();
DROP PROCEDURE IF EXISTS `__mig_V20260223b__remaining_tables_and_operator_fields`;
