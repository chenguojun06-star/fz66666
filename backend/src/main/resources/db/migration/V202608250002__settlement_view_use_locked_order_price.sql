-- D-128: 外发结算价格口径统一为「订单锁定单价」（下单时用户填写的价格）
-- 此前视图取款式报价 total_price（含利润的售价）兜底款式档案价，导致付给工厂的钱按售价而非加工费计算。
-- 现优先取 t_production_order.factory_unit_price（下单锁定单价），为 0/null 时才回退款式报价（兼容历史订单）。
-- 列表接口的 applyLockedOrderPrice 读时覆算逻辑保持不变（双保险，另可解析 orderDetails 快照价）。
...
SELECT `po`.`id` AS `order_id`,
       ...
       COALESCE(NULLIF(`po`.`factory_unit_price`, 0), `sq`.`total_price`, `si`.`price`, 0) AS `style_final_price`,
       ...
       ROUND(COALESCE(NULLIF(`po`.`factory_unit_price`, 0), `sq`.`total_price`, `si`.`price`, 0) * COALESCE(`wh`.`total_warehoused`, 0), 2) AS `total_amount`,
       ROUND((COALESCE(NULLIF(`po`.`factory_unit_price`, 0), `sq`.`total_price`, `si`.`price`, 0) * COALESCE(`wh`.`total_warehoused`, 0))
         - COALESCE(`mat`.`total_material_cost`, `po`.`material_cost`, 0) - COALESCE(`scan`.`total_production_cost`, 0)
         - (CASE WHEN (`po`.`order_quantity` > 0)
           THEN COALESCE(`wh`.`total_defects`, 0)
             * ((COALESCE(`mat`.`total_material_cost`, `po`.`material_cost`, 0) + COALESCE(`scan`.`total_production_cost`, 0))
             / `po`.`order_quantity`) ELSE 0 END), 2) AS `profit`,
       (CASE WHEN (COALESCE(NULLIF(`po`.`factory_unit_price`, 0), `sq`.`total_price`, `si`.`price`, 0) * COALESCE(`wh`.`total_warehoused`, 0)) > 0
         THEN ROUND(((COALESCE(NULLIF(`po`.`factory_unit_price`, 0), `sq`.`total_price`, `si`.`price`, 0) * COALESCE(`wh`.`total_warehoused`, 0))
           - COALESCE(`mat`.`total_material_cost`, `po`.`material_cost`, 0) - COALESCE(`scan`.`total_production_cost`,0)
           - (CASE WHEN (`po`.`order_quantity` > 0)
             THEN COALESCE(`wh`.`total_defects`,0)
               * ((COALESCE(`mat`.`total_material_cost`, `po`.`material_cost`, 0) + COALESCE(`scan`.`total_production_cost`, 0))
               / `po`.`order_quantity`) ELSE 0 END))
           / (COALESCE(NULLIF(`po`.`factory_unit_price`, 0), `sq`.`total_price`, `si`.`price`, 0) * COALESCE(`wh`.`total_warehoused`, 0)) * 100, 2)
