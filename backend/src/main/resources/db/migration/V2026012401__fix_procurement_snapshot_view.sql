...
SELECT
  ...
  MAX(CASE WHEN p.status <> 'pending' THEN p.received_time END) AS procurement_end_time,
  SUBSTRING_INDEX(
    MAX(CASE WHEN p.status <> 'pending' AND p.received_time IS NOT NULL THEN CONCAT(LPAD(UNIX_TIMESTAMP(p.received_time), 20, '0'), LPAD(UNIX_TIMESTAMP(p.create_time), 20, '0'), '|', IFNULL(p.receiver_name, '')) END),
    '|', -1
