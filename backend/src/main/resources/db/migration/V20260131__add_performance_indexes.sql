...
DROP PROCEDURE IF EXISTS `__mig_V20260131__add_performance_indexes`;
DELIMITER $$
CREATE PROCEDURE `__mig_V20260131__add_performance_indexes`()
BEGIN
    ...
    -- 订单编号索引（用于精确查询）
    CREATE INDEX idx_production_order_no
    ON t_production_order(order_no);

    -- 款式编号索引（用于关联查询）
    CREATE INDEX idx_production_style_no
    ON t_production_order(style_no);

    -- 工厂ID索引（用于工厂维度查询）
    CREATE INDEX idx_production_factory_id
    ...
    -- 创建时间索引（用于排序和范围查询）
    CREATE INDEX idx_production_create_time
    ON t_production_order(create_time);

    -- 复合索引：工厂+状态（常用查询组合）
    CREATE INDEX idx_production_factory_status
    ON t_production_order(factory_id, status);

    -- 复合索引：款式+创建时间（用于款式历史查询）
    CREATE INDEX idx_production_style_create
    ON t_production_order(style_id, create_time);

    -- 复合索引：状态+创建时间（用于状态筛选排序）
    CREATE INDEX idx_production_status_create
    ...
    -- 订单ID索引（用于聚合查询）
    CREATE INDEX idx_warehousing_order_id
    ON t_product_warehousing(order_id);

    -- 删除标记索引（用于软删除过滤）
    CREATE INDEX idx_warehousing_delete_flag
    ON t_product_warehousing(delete_flag);

    -- 复合索引：订单+删除标记（覆盖常用查询）
    CREATE INDEX idx_warehousing_order_delete
    ON t_product_warehousing(order_id, delete_flag);

    -- 复合索引：订单+删除标记+合格数量（覆盖聚合查询）
    CREATE INDEX idx_warehousing_order_delete_qualified
    ...
    -- 订单ID索引
    CREATE INDEX idx_outstock_order_id
    ON t_product_outstock(order_id);

    -- 删除标记索引
    CREATE INDEX idx_outstock_delete_flag
    ON t_product_outstock(delete_flag);

    -- 复合索引：订单+删除标记
    CREATE INDEX idx_outstock_order_delete
    ON t_product_outstock(order_id, delete_flag);

    -- 复合索引：订单+删除标记+出库数量
    CREATE INDEX idx_outstock_order_delete_quantity
    ...
    -- 生产订单ID索引
    CREATE INDEX idx_cutting_order_id
    ON t_cutting_bundle(production_order_id);

    -- 菲号索引（用于菲号查询）
    CREATE INDEX idx_cutting_bundle_no
    ON t_cutting_bundle(bundle_no);

    -- 复合索引：订单+状态
    CREATE INDEX idx_cutting_order_status
    ...
    -- 创建时间索引
    CREATE INDEX idx_style_create_time
    ON t_style_info(create_time);

    -- 分类索引
    CREATE INDEX idx_style_category
    ON t_style_info(category);

    -- 复合索引：状态+创建时间
    CREATE INDEX idx_style_status_create
    ...
    -- 生产订单ID索引
    CREATE INDEX idx_material_purchase_order_id
    ON t_material_purchase(production_order_id);

    -- 物料ID索引
    CREATE INDEX idx_material_purchase_material_id
    ON t_material_purchase(material_id);

    -- 状态索引
    CREATE INDEX idx_material_purchase_status
    ON t_material_purchase(status);

    -- 复合索引：订单+物料
    CREATE INDEX idx_material_purchase_order_material
    ...
    -- 款式ID索引
    CREATE INDEX idx_process_style_id
    ON t_process_info(style_id);

    -- 工序编号索引
    CREATE INDEX idx_process_no
    ON t_process_info(process_no);

    -- 复合索引：款式+工序顺序
    CREATE INDEX idx_process_style_sequence
    ...
    -- 生产订单ID索引
    CREATE INDEX idx_production_record_order_id
    ON t_production_record(production_order_id);

    -- 工序ID索引
    CREATE INDEX idx_production_record_process_id
    ON t_production_record(process_id);

    -- 日期索引（用于日期范围查询）
    CREATE INDEX idx_production_record_date
    ON t_production_record(record_date);

    -- 复合索引：订单+工序
    CREATE INDEX idx_production_record_order_process
    ON t_production_record(production_order_id, process_id);

    -- 复合索引：订单+日期
    CREATE INDEX idx_production_record_order_date
...
END$$
DELIMITER ;
CALL `__mig_V20260131__add_performance_indexes`();
DROP PROCEDURE IF EXISTS `__mig_V20260131__add_performance_indexes`;
