...
DROP PROCEDURE IF EXISTS `__mig_V26__add_scan_record_phase3_6_fields`;
DELIMITER $$
CREATE PROCEDURE `__mig_V26__add_scan_record_phase3_6_fields`()
BEGIN
    ...
    -- 添加索引优化查询性能
    CREATE INDEX idx_current_progress_stage ON t_scan_record(current_progress_stage);
    CREATE INDEX idx_assignment_id ON t_scan_record(assignment_id);

END$$
DELIMITER ;
CALL `__mig_V26__add_scan_record_phase3_6_fields`();
DROP PROCEDURE IF EXISTS `__mig_V26__add_scan_record_phase3_6_fields`;
