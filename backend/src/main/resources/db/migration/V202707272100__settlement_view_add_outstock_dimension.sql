-- 结算视图补充出库维度：出库数量、出库金额、当前库存
-- 在原视图基础上新增 outstock_quantity / outstock_amount / current_stock 字段
-- 关联铁律：财务数据链路闭环（入库→出库→库存全链路可视化）
...
LEFT JOIN (
    SELECT `order_no`,
      ...
      SUM(CASE WHEN `outstock_type` != 'reversal' THEN COALESCE(`total_amount`, 0) ELSE COALESCE(`total_amount`, 0) END) AS `total_outstock_amount`
