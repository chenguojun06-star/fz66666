...
DROP PROCEDURE IF EXISTS `__mig_V32__add_logistics_ecommerce_permissions`;
DELIMITER $$
CREATE PROCEDURE `__mig_V32__add_logistics_ecommerce_permissions`()
BEGIN
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION BEGIN END;
    -- 添加物流管理和电商管理权限
    INSERT IGNORE INTO t_permission (permission_code, permission_name, type, description, create_time, update_time) VALUES
    -- 物流管理权限
...
END$$
DELIMITER ;
CALL `__mig_V32__add_logistics_ecommerce_permissions`();
DROP PROCEDURE IF EXISTS `__mig_V32__add_logistics_ecommerce_permissions`;
