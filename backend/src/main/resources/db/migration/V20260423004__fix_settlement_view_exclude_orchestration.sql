-- 修复: v_finished_product_settlement 视图 scan 子查询未排除 orchestration 类型
-- 导致成品结算的生产成本虚高（orchestration 是系统自动编排记录，不应计入生产成本）
...
SELECT
    ...
    (CASE WHEN `po`.`order_quantity` > 0
        THEN ROUND(`wh`.`total_defects` * (`mat`.`total_material_cost` + `scan`.`total_production_cost`) / `po`.`order_quantity`, 2)
        ELSE 0 END) AS `defect_loss`,

    ROUND(COALESCE(`sq`.`total_price`, `si`.`price`, 0) * `wh`.`total_warehoused`, 2) AS `total_amount`,

    ROUND(COALESCE(`sq`.`total_price`, `si`.`price`, 0) * `wh`.`total_warehoused`
    ...
    (CASE WHEN COALESCE(`sq`.`total_price`, `si`.`price`, 0) * `wh`.`total_warehoused` > 0
        THEN ROUND(
            (COALESCE(`sq`.`total_price`, `si`.`price`, 0) * `wh`.`total_warehoused`
             - COALESCE(`mat`.`total_material_cost`, 0)
             - COALESCE(`scan`.`total_production_cost`, 0)
             - (CASE WHEN `po`.`order_quantity` > 0
                   THEN ROUND(`wh`.`total_defects` * (`mat`.`total_material_cost` + `scan`.`total_production_cost`) / `po`.`order_quantity`, 2)
                   ELSE 0 END))
            / (COALESCE(`sq`.`total_price`, `si`.`price`, 0) * `wh`.`total_warehoused`) * 100, 2)
        ELSE 0 END) AS `profit_margin`,

    COALESCE(`po`.`actual_end_date`, `wh`.`last_warehoused_time`) AS `complete_time`,

    `po`.`create_time`, `po`.`update_time`, `po`.`tenant_id`
