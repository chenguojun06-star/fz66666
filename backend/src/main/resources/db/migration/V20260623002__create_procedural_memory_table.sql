...
-- 2. 幂等插入初始 SOP 数据（使用存储过程）
DROP PROCEDURE IF EXISTS init_procedural_memory;
DELIMITER //
CREATE PROCEDURE init_procedural_memory()
BEGIN
    -- 只插入不存在的记录
