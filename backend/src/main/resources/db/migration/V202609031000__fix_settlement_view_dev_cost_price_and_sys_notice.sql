-- ======================================================================
-- V202609031000: 修复两个高频DB错误
--
-- 修复1: v_finished_product_settlement 视图重建 — 补回 dev_cost_price 字段
--   问题：V202609021000（add_complete_time）重建视图时漏掉了
--         V20260420002 已添加的 dev_cost_price 列
--   错误：Unknown column 'dev_cost_price' in 'field list'（GET /finished-settlement/list 500）
--   修复：在 target_profit_rate 之后重新插入 COALESCE(si.price, 0) AS dev_cost_price
--
-- 修复2: t_sys_notice.to_name 无默认值
--   问题：MindPushOrchestrator 定时任务 INSERT 时未提供 to_name 值
--   错误：Field 'to_name' doesn't have a default value（高频，每分钟多次）
--   修复：MODIFY COLUMN 为 DEFAULT ''
-- ======================================================================
...
SELECT
    ...
    -- 开发成本价（款式档案原始价格），V20260420002 已添加但 V202609021000 漏掉
    ...
    -- 完成时间：优先取订单实际结束时间，次选最晚入库操作时间（V202609021000 已添加）
...
-- ==================== 修复2: t_sys_notice.to_name 加默认值 ====================
-- 防止 MindPushOrchestrator 等定时任务 INSERT 时未传 to_name 导致的高频 SQL 异常
ALTER TABLE `t_sys_notice`
    MODIFY COLUMN `to_name` VARCHAR(100) DEFAULT '' COMMENT '接收者名称';
