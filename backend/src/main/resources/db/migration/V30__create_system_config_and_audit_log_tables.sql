...
DROP PROCEDURE IF EXISTS `__mig_V30__create_system_config_and_audit_log_tables`;
DELIMITER $$
CREATE PROCEDURE `__mig_V30__create_system_config_and_audit_log_tables`()
BEGIN
    ...
    -- 插入默认系统配置
    INSERT IGNORE INTO t_system_config (config_key, config_name, config_value, config_type, category, description, editable, is_system, sort_order) VALUES
...
END$$
DELIMITER ;
CALL `__mig_V30__create_system_config_and_audit_log_tables`();
DROP PROCEDURE IF EXISTS `__mig_V30__create_system_config_and_audit_log_tables`;
