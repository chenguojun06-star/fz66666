 ...
 * <p>全部查询强制 tenant_id（P0 铁律 4）；工厂账号不返回租户级聚合（数据隔离）。</p>
 */
@Service
...
public class OrderAnalyticsOrchestrator {
    ...
    public OrderAnalyticsVO getAnalytics(int days) {
        ...
        vo.setOverview(buildOverview(tenantId, since));
        vo.setTrend(buildTrend(tenantId, since));
        vo.setFactoryRanking(buildFactoryRanking(tenantId, since));
        vo.setDefectRanking(buildDefectRanking(tenantId, since));
        vo.setMargin(buildMargin(tenantId, since));
        return vo;
    }
    ...
    private OrderAnalyticsVO.Overview buildOverview(Long tenantId, LocalDateTime since) {
        ...
        try {
            jdbcTemplate.query(
                    "SELECT COUNT(*) AS orderCount, " +
                            ...
                            "FROM t_production_order WHERE tenant_id = ? AND delete_flag = 0 AND create_time >= ?",
                    rs -> {
            ...
            o.setAvgCompletionDays(round1(queryAvgCompletionDays(tenantId, since)));
            o.setAvgDefectRate(round1(queryAvgDefectRate(tenantId, since)));
        } catch (Exception e) {
            log.warn("[订单分析] 总览统计失败: {}", e.getMessage());
        }
        return o;
    }

    private double queryAvgCompletionDays(Long tenantId, LocalDateTime since) {
        Double v = jdbcTemplate.queryForObject(
                ...
                Double.class, tenantId, since);
        return v == null ? -1 : v;
    }

    private double queryAvgDefectRate(Long tenantId, LocalDateTime since) {
    ...
    private List<OrderAnalyticsVO.TrendItem> buildTrend(Long tenantId, LocalDateTime since) {
        try {
            return jdbcTemplate.query(
                    ...
                    (rs, i) -> OrderAnalyticsVO.TrendItem.builder()
                            ...
                            .build(), tenantId, since);
        } catch (Exception e) {
            log.warn("[订单分析] 趋势统计失败: {}", e.getMessage());
            return new ArrayList<>();
        }
    }

    /** ③ 工厂时效排行：平均完工天数 + 准时交付率（按完成单统计） */
    private List<OrderAnalyticsVO.FactoryRankItem> buildFactoryRanking(Long tenantId, LocalDateTime since) {
        try {
            ...
            log.warn("[订单分析] 工厂时效统计失败: {}", e.getMessage());
            return new ArrayList<>();
        }
    }

    /** ④ 次品率排行：按款号聚合质检扫码 */
    private List<OrderAnalyticsVO.DefectRankItem> buildDefectRanking(Long tenantId, LocalDateTime since) {
        try {
            ...
            log.warn("[订单分析] 次品率统计失败: {}", e.getMessage());
            return new ArrayList<>();
        }
    }

    /** ⑤ 毛利估算：销售额 - 总成本（优先 total_cost，兜底 material_cost） */
    private OrderAnalyticsVO.Margin buildMargin(Long tenantId, LocalDateTime since) {
        ...
        try {
            ...
            log.warn("[订单分析] 毛利统计失败: {}", e.getMessage());
        }
        return m;
    }
