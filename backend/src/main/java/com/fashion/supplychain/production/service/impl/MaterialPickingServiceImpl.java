            ...
            for (MaterialPickingItem item : items) {
                ...
                try {
                    ...
                    if (stock != null) {
                        ...
                        log.warn("[Picking] 库存记录未找到: materialId={}, materialCode={}, color={}, size={}",
                                item.getMaterialId(), item.getMaterialCode(), item.getColor(), item.getSize());
                    }
                } catch (Exception e) {
                    log.warn("[Picking] 查询库存记录异常: materialId={}, error={}", item.getMaterialId(), e.getMessage());
                }
                materialPickingItemMapper.insert(item);
            }
        }
