...
public class CuttingBundleServiceImpl extends ServiceImpl<CuttingBundleMapper, CuttingBundle>
    ...
    public List<CuttingBundle> generateBundles(String orderId, List<Map<String, Object>> bundles) {
        ...
        if (!"received".equals(task.getStatus())) {
            if ("bundled".equals(task.getStatus())) {
                throw new IllegalStateException("该订单已生成裁剪单");
            }
    ...
    public Map<String, Object> summarize(String orderNo, String orderId) {
        ...
        if (StringUtils.hasText(on)) {
            qw.lambda().eq(CuttingBundle::getProductionOrderNo, on);
        }
        if (StringUtils.hasText(oid)) {
            qw.lambda().eq(CuttingBundle::getProductionOrderId, oid);
        }
        ...
        int bundleCount = 0;
        if (first != null) {
            totalQuantity = ParamUtils.toIntSafe(ParamUtils.getIgnoreCase(first, "totalQuantity"));
            bundleCount = ParamUtils.toIntSafe(ParamUtils.getIgnoreCase(first, "bundleCount"));
        }

        Map<String, Object> data = new HashMap<>();
        data.put("totalQuantity", totalQuantity);
        data.put("bundleCount", bundleCount);
        return data;
    }
