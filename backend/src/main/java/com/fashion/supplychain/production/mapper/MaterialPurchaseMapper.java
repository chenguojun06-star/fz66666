...
public interface MaterialPurchaseMapper extends BaseMapper<MaterialPurchase> {
    ...
    @Select({
            ...
            "  MIN(CASE WHEN p.status = 'completed' OR p.received_time IS NOT NULL THEN COALESCE(p.received_time, p.actual_arrival_date, p.create_time) END) AS procurementStartTime,",
            "  MAX(CASE WHEN p.status = 'completed' THEN COALESCE(p.actual_arrival_date, p.received_time) END) AS procurementEndTime,",
            "  SUBSTRING_INDEX(",
