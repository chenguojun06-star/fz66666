...
public interface ScanRecordMapper extends BaseMapper<ScanRecord> {
        ...
        @Select({
                        "<script>",
                        "SELECT",
                        "  COUNT(CASE WHEN process_name NOT LIKE '%采购%' AND process_name NOT LIKE '%物料%' AND scan_type != 'procurement' THEN 1 END) AS scanCount,",
                        "  COUNT(DISTINCT CASE WHEN process_name NOT LIKE '%采购%' AND process_name NOT LIKE '%物料%' AND scan_type != 'procurement' THEN order_id END) AS orderCount,",
                        "  COALESCE(SUM(CASE WHEN process_name NOT LIKE '%采购%' AND process_name NOT LIKE '%物料%' AND scan_type != 'procurement' THEN quantity ELSE 0 END), 0) AS totalQuantity,",
                        "  COALESCE(SUM(CASE WHEN process_name NOT LIKE '%采购%' AND process_name NOT LIKE '%物料%' AND scan_type != 'procurement' THEN total_amount ELSE 0 END), 0) AS totalAmount",
                        "FROM t_scan_record",
