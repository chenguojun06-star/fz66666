...
public class SKUServiceImpl implements SKUService {
    ...
    public List<Map<String, Object>> getProcessUnitPrices(String orderNo) {
        ...
        try {
            ...
            result = parseWorkflowUnitPrices(orderNo, workflowJson);
            mergeProgressStageFromTemplate(result, order.getStyleNo());
            applyScanTypeInference(result);
    ...
    private List<Map<String, Object>> parseWorkflowUnitPrices(String orderNo, String workflowJson) throws Exception {
        List<Map<String, Object>> result = new ArrayList<>();
        ...
        for (Map<String, Object> node : nodes) {
            ...
            try {
                ...
                if (StringUtils.hasText(processId)) priceInfo.put("id", processId);
                if (StringUtils.hasText(processName)) {
                    priceInfo.put("name", processName);
                    priceInfo.put("processName", processName);
                }
                if (StringUtils.hasText(progressStage)) priceInfo.put("progressStage", progressStage);
    ...
    private void mergeProgressStageFromTemplate(List<Map<String, Object>> result, String styleNo) {
        if (templateLibraryService == null || styleNo == null) return;
        try {
    ...
    private void applyScanTypeInference(List<Map<String, Object>> result) {
        for (Map<String, Object> r : result) {
            String pn = r.get("processName") != null ? r.get("processName").toString() : "";
    ...
    private java.math.BigDecimal resolveProcessUnitPrice(String orderNo, String processName, String progressStage) {
        ...
        if (unitPrice.compareTo(java.math.BigDecimal.ZERO) <= 0 && StringUtils.hasText(progressStage)) {
            String stageName = progressStage.trim();
            if (!stageName.equalsIgnoreCase(processName.trim())) {
                Map<String, Object> stagePriceInfo = getUnitPriceByProcess(orderNo, stageName);
                java.math.BigDecimal stagePrice = parseBigDecimal(stagePriceInfo.get("unitPrice"));
