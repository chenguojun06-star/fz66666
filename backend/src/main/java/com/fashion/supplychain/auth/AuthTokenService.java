...
public class AuthTokenService {
    ...
    public String issueToken(TokenSubject subject, Duration ttl) {
        ...
        if (subject.getPosition() != null) {
            payload.put("position", subject.getPosition()); // 职位
        }
    ...
    public TokenSubject verifyAndParse(String token) {
        ...
        Object positionObj = jwt.getPayload("position");
        if (positionObj != null) {
            subject.setPosition(String.valueOf(positionObj));
        }
        Object superAdminObj = jwt.getPayload("superAdmin");
        subject.setSuperAdmin(Boolean.TRUE.equals(superAdminObj));
        // 解析密码版本号
        Object pwdVerObj = jwt.getPayload("pwdVer");
        if (pwdVerObj != null) {
            try { subject.setPwdVersion(Long.valueOf(String.valueOf(pwdVerObj))); }
            catch (NumberFormatException e) { subject.setPwdVersion(0L); }
        } else {
