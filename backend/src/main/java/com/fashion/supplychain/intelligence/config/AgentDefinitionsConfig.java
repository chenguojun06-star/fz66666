package com.fashion.supplychain.intelligence.config;

import java.util.List;
import java.util.Map;

/**
 * Agent 定义配置 — 30个智能体 + 工具映射表
 *
 * 从 AgentActivityController 提取，避免 483 行中 ~150 行是静态数据
 */
public final class AgentDefinitionsConfig {

    private AgentDefinitionsConfig() {}

    public static final List<AgentDefinition> AGENT_DEFINITIONS = List.of(
            def("order-manager", "订单管家", "production", "#1677ff",
                    "管理生产订单全生命周期：创建、编辑、转厂、催单、学习、对比"),
            def("material-buyer", "物料采购员", "production", "#52c41a",
                    "面辅料采购：到货入库、领料、对账、采购单管理、供应商管理"),
            def("quality-inspector", "质检巡检员", "production", "#faad14",
                    "成品质检入库、次品返修报废、工资异常检测"),
            def("production-scheduler", "生产调度员", "production", "#ff7a45",
                    "生产进度查询、异常上报、裁剪单创建、拆菲转派"),
            def("finance-settler", "财务结算员", "finance", "#eb2f96",
                    "财务审批付款、工资结算审批、出货对账、发票管理、税务配置"),
            def("warehouse-keeper", "仓库管理员", "warehouse", "#722ed1",
                    "库存管理：物料库存查询、成品库存、样衣借还、盘点、操作日志"),
            def("inventory-manager", "出入库专员", "warehouse", "#fa8c16",
                    "出入库操作：面辅料收货入库、成品出库、采购到货、领料管理"),
            def("style-designer", "样衣开发员", "basic", "#13c2c2",
                    "样衣开发与纸样管理：款式建档、模板、难度评估、报价、二次加工"),
            def("data-analyst", "数据分析师", "intelligence", "#fa541c",
                    "深度分析、延期趋势、供应商评分、智能报表、管理看板、系统概览"),
            def("risk-sentinel", "风险哨兵", "intelligence", "#f5222d",
                    "根因分析、人员延期分析、变更审批、生产异常上报"),
            def("smart-advisor", "智能顾问", "intelligence", "#2f54eb",
                    "AI对话：知识搜索、多代理协同、Agent例会、团队分派"),
            def("learning-engine", "学习引擎", "intelligence", "#52c41a",
                    "自主学习：规律发现、目标拆解、Critic进化、场景模拟、自我优化"),
            def("system-doctor", "系统医生", "intelligence", "#9254de",
                    "系统诊断与自愈：代码诊断、组织查询、用户管理、字典维护"),
            def("patrol-sentinel", "巡逻哨兵", "intelligence", "#e65100",
                    "自动巡检：逾期扫描、停滞检测、主动诊断、智能备注推送"),
            def("sourcing-specialist", "采购专家", "intelligence", "#00897b",
                    "BOM成本分析、供应商交付评估、物料缺口识别"),
            def("delivery-specialist", "交付专家", "intelligence", "#d84315",
                    "交付风险评估、订单健康评分、逾期预警"),
            def("compliance-specialist", "合规专家", "intelligence", "#6a1b9a",
                    "质量合格率分析、缺陷追踪、DPP合规检查"),
            def("logistics-specialist", "物流专家", "intelligence", "#1565c0",
                    "库存水位分析、出入库节奏、物流延迟风险"),
            def("crew-coordinator", "生产协调员", "production", "#00838f",
                    "自然语言命令解析、批量生产建议、订单只读分析"),
            def("evolution-engine", "进化引擎", "intelligence", "#ad1457",
                    "GitHub技术研究、反馈驱动进化、自我优化提案"),
            def("insight-generator", "洞察生成器", "intelligence", "#00695c",
                    "每日洞察简报、晨报生成、业务趋势分析"),
            def("smart-remark", "智能备注员", "intelligence", "#5d4037",
                    "自动备注：订单备注巡检、智能备注推送、异常标记"),
            def("critic-agent", "批评检查官", "intelligence", "#4e342e",
                    "审查AI输出：数据溯源校验、逻辑一致性检查、遗漏检测"),
            def("hyper-advisor", "超级顾问", "intelligence", "#1a237e",
                    "深度推演、风险模拟、知识收割、安全建议"),
            def("forecast-engine", "预测引擎", "intelligence", "#0d47a1",
                    "交付预测：交期建议、销售预测、进度预测"),
            def("visual-ai", "视觉AI", "intelligence", "#311b92",
                    "图像分析：图片识别、文件分析、视觉质检"),
            def("voice-command", "语音指令", "intelligence", "#4a148c",
                    "语音交互：语音命令解析、语音转文字、语音操作"),
            def("self-healing", "自愈引擎", "intelligence", "#b71c1c",
                    "数据修复：一致性诊断、自动修复、孤儿数据检测"),
            def("intelligence-brain", "智能中枢", "intelligence", "#1b5e20",
                    "大脑快照：健康度聚合、风险脉搏、异常感知、学习闭环"),
            def("anomaly-detector", "异常检测器", "intelligence", "#880e4f",
                    "异常检测：对账异常、工厂瓶颈、物料短缺、停滞预警")
    );

    public static final Map<String, List<String>> DOMAIN_TOOLS = Map.ofEntries(
            Map.entry("order-manager", List.of("tool_order_edit", "tool_order_contact_urge",
                    "tool_order_factory_transfer", "tool_order_factory_transfer_undo",
                    "tool_order_learning", "tool_query_order_remarks",
                    "tool_create_production_order", "tool_simulate_new_order",
                    "tool_query_crm_customer", "orderStatusCheck")),
            Map.entry("material-buyer", List.of("tool_material_receive", "tool_material_doc_receive",
                    "tool_material_reconciliation", "tool_procurement",
                    "tool_material_audit", "tool_material_calculation",
                    "tool_material_picking", "materialBuyerCheck", "materialBuyerReconciliation")),
            Map.entry("quality-inspector", List.of("tool_quality_inbound", "tool_defective_board",
                    "tool_payroll_anomaly_detect", "tool_payroll_approve",
                    "tool_query_financial_payroll", "qualityInspectorCheck", "defectInspection")),
            Map.entry("production-scheduler", List.of("tool_query_production_progress", "tool_production_exception",
                    "tool_cutting_task_create", "tool_bundle_split_transfer",
                    "tool_team_dispatch", "tool_action_executor", "tool_order_batch_close",
                    "tool_scheduling_suggestion", "productionSchedulerCheck", "scheduleOptimization")),
            Map.entry("finance-settler", List.of("tool_finance_workflow", "tool_shipment_reconciliation",
                    "tool_payroll_approve", "tool_query_financial_payroll",
                    "tool_payroll_anomaly_detect", "financeSettlerCheck", "paymentReconciliation")),
            Map.entry("warehouse-keeper", List.of("tool_query_warehouse_stock", "tool_warehouse_op_log",
                    "tool_finished_product_stock", "tool_sample_stock",
                    "tool_sample_loan", "tool_scan_undo",
                    "warehouseKeeperCheck", "stockAudit")),
            Map.entry("inventory-manager", List.of("tool_material_receive", "tool_material_doc_receive",
                    "tool_finished_outbound", "tool_procurement",
                    "tool_material_picking", "tool_material_audit",
                    "inventoryManagerCheck", "inventoryFlowCheck")),
            Map.entry("style-designer", List.of("tool_query_style_info", "tool_style_template",
                    "tool_query_style_difficulty", "tool_sample_workflow",
                    "tool_sample_delay_analysis", "tool_pattern_production",
                    "tool_secondary_process", "styleProgressCheck", "styleDifficultyAnalysis")),
            Map.entry("data-analyst", List.of("tool_deep_analysis", "tool_delay_trend",
                    "tool_supplier_scorecard", "tool_smart_report",
                    "tool_system_overview", "tool_scenario_simulator",
                    "tool_whatif", "dataTrendAnalysis", "supplierPerformance")),
            Map.entry("risk-sentinel", List.of("tool_root_cause_analysis", "tool_personnel_delay_analysis",
                    "tool_change_approval", "tool_production_exception",
                    "tool_order_contact_urge", "tool_anomaly_detection",
                    "tool_delivery_prediction", "scanOverdue", "scanStagnant",
                    "smartRemark", "proactiveDiagnose", "riskSentinelCheck", "riskEarlyWarning")),
            Map.entry("smart-advisor", List.of("tool_knowledge_search", "tool_multi_agent",
                    "tool_agent_meeting", "tool_team_dispatch",
                    "tool_action_executor", "tool_think", "smartAdvisorConsult", "knowledgeQuery")),
            Map.entry("learning-engine", List.of("tool_pattern_discovery", "tool_goal_decompose",
                    "tool_critic_evolution", "tool_order_learning",
                    "tool_scenario_simulator", "learningEngineCycle", "patternMining")),
            Map.entry("system-doctor", List.of("tool_code_diagnostic", "tool_org_query",
                    "tool_query_system_user", "tool_action_executor",
                    "systemDoctorCheck", "systemHealthScan")),
            Map.entry("patrol-sentinel", List.of("ai-patrol", "scanOverdue", "scanStagnant",
                    "smartRemark", "proactiveDiagnose", "tool_production_exception",
                    "tool_order_contact_urge", "patrolSentinelDuty", "overdueAlert")),
            Map.entry("sourcing-specialist", List.of("tool_material_calculation",
                    "tool_supplier_scorecard", "tool_procurement", "tool_supplier",
                    "sourcingSpecialistCheck", "supplierDeliveryAssessment", "materialGapAnalysis")),
            Map.entry("delivery-specialist", List.of("tool_delivery_prediction",
                    "tool_delay_trend", "tool_system_overview", "tool_deep_analysis",
                    "tool_query_production_progress", "deliverySpecialistCheck", "deliveryRiskForecast")),
            Map.entry("compliance-specialist", List.of("tool_quality_inbound",
                    "tool_defective_board", "tool_payroll_anomaly_detect",
                    "complianceSpecialistCheck", "qualityRateAnalysis", "defectTracking")),
            Map.entry("logistics-specialist", List.of("tool_query_warehouse_stock",
                    "tool_finished_product_stock", "tool_material_receive",
                    "tool_finished_outbound", "logisticsSpecialistCheck", "inventoryWaterLevelAnalysis")),
            Map.entry("crew-coordinator", List.of("tool_action_executor",
                    "tool_team_dispatch", "tool_query_production_progress",
                    "tool_system_overview", "crewCoordinatorCheck", "teamEfficiency")),
            Map.entry("evolution-engine", List.of("tool_critic_evolution",
                    "tool_ai_self_optimize_report", "tool_pattern_discovery",
                    "evolutionEngineCycle", "feedbackDrivenOptimization")),
            Map.entry("insight-generator", List.of("tool_smart_report",
                    "tool_system_overview", "tool_deep_analysis", "tool_delay_trend",
                    "insightGeneratorCheck", "dailyBriefing", "businessTrendMining")),
            Map.entry("smart-remark", List.of("smartRemark", "tool_query_order_remarks",
                    "smartRemarkCheck", "orderRemarkAnalysis")),
            Map.entry("critic-agent", List.of("tool_critic_evolution", "tool_think",
                    "criticAgentCheck", "outputQualityAudit")),
            Map.entry("hyper-advisor", List.of("tool_whatif", "tool_scenario_simulator",
                    "tool_deep_analysis", "tool_knowledge_search",
                    "hyperAdvisorCheck", "deepDeduction", "riskSimulation")),
            Map.entry("forecast-engine", List.of("tool_delivery_prediction",
                    "tool_system_overview", "tool_smart_report", "tool_delay_trend", "tool_whatif",
                    "forecastEngineCheck", "deliveryDateForecast", "salesForecast")),
            Map.entry("visual-ai", List.of("tool_query_style_info",
                    "tool_quality_inbound", "tool_defective_board",
                    "visualAICheck", "imageRecognition", "qualityPhotoAnalysis")),
            Map.entry("voice-command", List.of("tool_action_executor", "tool_think",
                    "voiceCommandCheck", "voiceToText")),
            Map.entry("self-healing", List.of("tool_code_diagnostic", "tool_org_query",
                    "selfHealingCheck", "orphanDataCleanup", "consistencyDiagnosis")),
            Map.entry("intelligence-brain", List.of("tool_system_overview",
                    "tool_deep_analysis", "tool_smart_report",
                    "tool_delivery_prediction", "tool_scheduling_suggestion",
                    "tool_anomaly_detection",
                    "intelligenceBrainCheck", "unifiedHealthAggregation", "riskPulse")),
            Map.entry("anomaly-detector", List.of("tool_anomaly_detection",
                    "tool_production_exception", "tool_material_calculation", "tool_delay_trend",
                    "tool_payroll_anomaly_detect",
                    "anomalyDetectorCheck", "factoryBottleneck", "materialShortageAlert"))
    );

    private static AgentDefinition def(String id, String name, String department, String color, String description) {
        return new AgentDefinition(id, name, department, color, description);
    }

    public static class AgentDefinition {
        public final String id;
        public final String name;
        public final String department;
        public final String color;
        public final String description;

        AgentDefinition(String id, String name, String department, String color, String description) {
            this.id = id; this.name = name; this.department = department;
            this.color = color; this.description = description;
        }
    }
}