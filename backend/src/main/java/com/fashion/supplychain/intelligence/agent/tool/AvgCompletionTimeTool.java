...
public class AvgCompletionTimeTool extends AbstractAgentTool {
    ...
    public AiTool getToolDefinition() {
        ...
        properties.put("factoryId", prop("string", "工厂ID，筛选特定工厂的完成时间"));
        properties.put("category", prop("string", "品类，如针织/梭织等"));
        properties.put("sampleSize", prop("integer", "统计样本量(默认30，最大100)"));
    ...
    protected String doExecute(String argumentsJson) throws Exception {
        ...
        if (tenantId == null) {
            return errorJson("租户上下文丢失");
        }

        Map<String, Object> args = parseArgs(argumentsJson);
        String factoryId = optionalString(args, "factoryId");
        String category = optionalString(args, "category");
        int sampleSize = 30;
        Integer ss = optionalInt(args, "sampleSize");
        if (ss != null) {
            sampleSize = Math.min(Math.max(ss, 10), 100);
        }
