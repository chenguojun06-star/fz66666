...
public interface CollaborationTaskMapper extends BaseMapper<CollaborationTask> {
    ...
    @Update("UPDATE t_collaboration_task SET task_status = 'ESCALATED', escalated_at = NOW(), escalated_to = #{escalatedTo}, updated_at = NOW() WHERE id = #{id} AND tenant_id = #{tenantId}")
    int escalateTask(@Param("id") Long id, @Param("tenantId") Long tenantId, @Param("escalatedTo") String escalatedTo);
}
