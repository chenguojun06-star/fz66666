...
final class DbViewRepairHelper {
    ...
    static int ensureSettlementViewHasCompleteTime(Connection conn, String schema) {
        try {
            ...
            boolean missingOutstockQuantity = !columnExists(conn, schema, "v_finished_product_settlement", "outstock_quantity");
            boolean missingFactoryType = !columnExists(conn, schema, "v_finished_product_settlement", "factory_type");
            if (missingCompleteTime || missingDevCostPrice || missingOutstockQuantity || missingFactoryType) {
                try (Statement stmt = conn.createStatement()) {
                    String createView = "CREATE OR REPLACE VIEW `v_finished_product_settlement` AS"
                        ...
                        + " LEFT JOIN (SELECT `order_no`,SUM(COALESCE(`total_amount`,0)) AS `total_material_cost`"
                        ...
                        + "   FROM `t_product_outstock` WHERE `delete_flag`=0"
                        + "     AND (`reversal_status` IS NULL OR `reversal_status` != 'REVERSED')"
                        + "   GROUP BY `order_no`) `out` ON `po`.`order_no`=`out`.`order_no`"
