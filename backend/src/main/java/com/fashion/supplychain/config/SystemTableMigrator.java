...
public class SystemTableMigrator {
    ...
    private void fixAppStorePrices(JdbcTemplate jdbc) {
        ...
        try {
            ...
            if (zeroCount != null && zeroCount > 0) {
                ...
                jdbc.update("UPDATE t_app_store SET price_monthly=299.00,  price_yearly=2990.00, price_once=19999.00 WHERE app_code='EC_SHEIN'");
    ...
    private void fixAppStoreEncoding(JdbcTemplate jdbc) {
        ...
        try {
            ...
            jdbc.update("UPDATE t_app_store SET app_name='希音', app_desc='对接希音平台，自动同步订单数据，实时跟踪订单状态', features='[\"订单同步\",\"状态跟踪\",\"发货管理\",\"库存同步\"]' WHERE app_code='EC_SHEIN'");
