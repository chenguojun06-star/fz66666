...
public class ProductionTableMigrator {
    ...
    public void initialize() {
        ...
        ensureCuttingBundleSplitLogTable();
    ...
    private void ensureCuttingBundleTable() {
        ...
        if (!dbHelper.columnExists("t_cutting_bundle", "split_status")) {
            dbHelper.execSilently("ALTER TABLE t_cutting_bundle ADD COLUMN split_status VARCHAR(20) NOT NULL DEFAULT 'normal' COMMENT 'normal/split_parent/split_child'");
        }
    ...
    private void ensureCuttingBundleSplitLogTable() {
        ...
        if (!dbHelper.columnExists("t_cutting_bundle_split_log", "rollback_time")) {
            dbHelper.execSilently("ALTER TABLE t_cutting_bundle_split_log ADD COLUMN rollback_time DATETIME DEFAULT NULL");
        }
        if (!dbHelper.columnExists("t_cutting_bundle_split_log", "rollback_by")) {
            dbHelper.execSilently("ALTER TABLE t_cutting_bundle_split_log ADD COLUMN rollback_by VARCHAR(50) DEFAULT NULL");
        }
