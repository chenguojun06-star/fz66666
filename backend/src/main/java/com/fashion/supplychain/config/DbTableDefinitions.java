...
public final class DbTableDefinitions {
    ...
    public static void addMissingTables() {
        ...
        TABLE_FIXES.put("t_order_image_snapshot",
            "CREATE TABLE IF NOT EXISTS `t_order_image_snapshot` ("
            + "`id` BIGINT NOT NULL AUTO_INCREMENT,"
            + "`order_no` VARCHAR(64) NOT NULL COMMENT '订单号',"
            + "`snapshot_type` VARCHAR(20) NOT NULL COMMENT '快照类型',"
            + "`before_urls` TEXT DEFAULT NULL COMMENT '变更前URLs',"
            + "`after_urls` TEXT DEFAULT NULL COMMENT '变更后URLs',"
            + "`operator_id` VARCHAR(36) DEFAULT NULL COMMENT '操作人ID',"
            + "`operator_name` VARCHAR(64) DEFAULT NULL COMMENT '操作人姓名',"
            + "`tenant_id` BIGINT NOT NULL COMMENT '租户ID',"
            + "`create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',"
            ...
            + "KEY `idx_order_no` (`order_no`),"
            + "KEY `idx_tenant_id` (`tenant_id`)"
            + ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='订单图片快照'");
        ...
        TABLE_FIXES.put("t_warehouse_area",
            ...
            + ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
        TABLE_FIXES.put("t_ec_product_mapping",
        ...
        TABLE_FIXES.put("t_ec_sync_log",
            ...
            + ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
        TABLE_FIXES.put("t_ec_sync_config",
        ...
        TABLE_FIXES.put("t_cutting_bom",
            ...
            + ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
        TABLE_FIXES.put("t_bargain_price",
            "CREATE TABLE IF NOT EXISTS `t_bargain_price` ("
            + "`id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键',"
            + "`target_type` VARCHAR(32) DEFAULT NULL COMMENT '还价目标类型',"
            + "`target_id` VARCHAR(64) DEFAULT NULL COMMENT '还价目标ID',"
            + "`target_no` VARCHAR(100) DEFAULT NULL COMMENT '还价目标编号',"
            + "`original_price` DECIMAL(12,2) DEFAULT NULL COMMENT '原始单价',"
            + "`bargained_price` DECIMAL(12,2) DEFAULT NULL COMMENT '还价单价',"
            + "`reason` VARCHAR(500) DEFAULT NULL COMMENT '还价原因',"
            + "`bargained_by` VARCHAR(64) DEFAULT NULL COMMENT '还价人ID',"
            + "`bargained_by_name` VARCHAR(100) DEFAULT NULL COMMENT '还价人姓名',"
            + "`approved_by` VARCHAR(64) DEFAULT NULL COMMENT '审批人ID',"
            + "`approved_by_name` VARCHAR(100) DEFAULT NULL COMMENT '审批人姓名',"
            + "`status` VARCHAR(20) DEFAULT NULL COMMENT '状态',"
            + "`create_time` DATETIME DEFAULT NULL COMMENT '创建时间',"
        ...
        TABLE_FIXES.put("t_employee_advance",
            ...
            + "`approval_remark` VARCHAR(500) DEFAULT NULL COMMENT '审批备注',"
            + "`repayment_amount` DECIMAL(12,2) DEFAULT NULL COMMENT '已还金额',"
            + "`remaining_amount` DECIMAL(12,2) DEFAULT NULL COMMENT '剩余金额',"
            + "`repayment_status` VARCHAR(20) DEFAULT NULL COMMENT '还款状态',"
            + "`create_time` DATETIME DEFAULT NULL COMMENT '创建时间',"
            + "`update_time` DATETIME DEFAULT NULL COMMENT '更新时间',"
            ...
            + "`tenant_id` BIGINT DEFAULT NULL COMMENT '租户ID',"
            + "PRIMARY KEY (`id`),"
            + "KEY `idx_ea_tenant_employee` (`tenant_id`, `employee_id`),"
            + "KEY `idx_ea_tenant_status` (`tenant_id`, `status`)"
            + ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='员工预支记录表'");

        TABLE_FIXES.put("t_purchase_cart",
            ...
            + "PRIMARY KEY (`id`),"
            + "KEY `idx_pc_tenant_user` (`tenant_id`, `user_id`)"
            + ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='采购购物车表'");

        TABLE_FIXES.put("t_purchase_cart_item",
            ...
            + "`source_quantity` DECIMAL(12,2) DEFAULT NULL,"
            + "`color` VARCHAR(50) DEFAULT NULL,"
            + "`fabric_composition` VARCHAR(200) DEFAULT NULL,"
            + "`fabric_width` VARCHAR(50) DEFAULT NULL,"
            + "`fabric_weight` VARCHAR(50) DEFAULT NULL,"
            + "`merge_group_id` VARCHAR(64) DEFAULT NULL,"
            + "`remark` VARCHAR(500) DEFAULT NULL,"
            + "`sort_order` INT DEFAULT 0,"
            + "`delete_flag` INT DEFAULT 0,"
            + "`created_time` DATETIME DEFAULT CURRENT_TIMESTAMP,"
