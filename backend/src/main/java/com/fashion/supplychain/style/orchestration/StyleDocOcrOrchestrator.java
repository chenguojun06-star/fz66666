    ...
    public Map<String, Object> recognizeBomTable(MultipartFile file) {
        ...
        try {
            String prompt = "你是专业服装BOM物料清单识别助手。请仔细识别图片中的物料清单。\n" +
                    ...
                    "4. 部位（如：整件/上装/下装/马甲/里布，没有明确标注则填整件）\n" +
                    "5. 子部位（如：袖口/领子/门襟/下摆/口袋/腰头/裤脚；如工艺单未标注子部位则填空字符串）\n" +
                    "6. 颜色（如图片中有颜色信息则提取）\n" +
                    "7. 规格/型号（如：20D/210T、YKK-5号等）\n" +
                    "8. 单位（米/码/个/粒/卷/公斤等）\n" +
                    "9. 单件用量（数字，不含单位）\n" +
                    "10. 损耗率（百分比数字，如5表示5%）\n" +
                    "11. 供应商（如图片中有则提取）\n" +
                    ...
                    "      \"partName\": \"整件|上装|下装|马甲|里布（没有明确标注则填整件）\",\n" +
                    "      \"subPartName\": \"袖口|领子|门襟|下摆|口袋|腰头|裤脚（没有明确标注则填空字符串）\",\n" +
                    "      \"color\": \"颜色（没有则为空字符串）\",\n" +
                    ...
                    "- 部位 partName 必须是以下之一：整件、上装、下装、马甲、里布；如工艺单未标注部位则填整件\n" +
                    "- 子部位 subPartName 如袖口/领子/门襟/下摆/口袋等，未标注则填空字符串\n" +
