...
const useStyleDevelopmentWorkbenchData = ({ record, initialSection, onSync }: Pick<Props, 'record' | 'initialSection' | 'onSync'>) => {
  ...
  const stageCards = useMemo<StageCard[]>(() => {
    ...
    return [
      { key: 'bom', title: '物料清单', count: `${data.bomList.length} 项`, meta: bomMeta, helper: formatTime((detail as any).bomCompletedTime || (detail as any).bomStartTime), availableTime: createTime, startTime: (detail as any).bomStartTime, endTime: (detail as any).bomCompletedTime, completed: Boolean((detail as any).bomCompletedTime), budgetHours: (detail as any).bomBudgetHours ?? DEFAULT_BUDGET_HOURS, budgetField: 'bomBudgetHours' as const, budgetCustomized: (detail as any).bomBudgetHours != null },
