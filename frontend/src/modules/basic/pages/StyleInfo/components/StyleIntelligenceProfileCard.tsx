    ...
    >
      ...
      {expanded && (
        <div style={{ padding: '0 12px 8px' }}>
          {/* 基本信息行 */}
          <div style={{ color: '#8c8c8c', fontSize: 13, lineHeight: 1.6, marginBottom: 6, borderBottom: '1px solid rgba(0,0,0,0.04)', paddingBottom: 5 }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: '0 0 42%', minWidth: 0 }}>
                ...
                {[
                  ...
                  <div key={item.key} style={{ padding: '5px 7px', borderRadius: 6, background: '#fff', border: '1px solid rgba(0,0,0,0.06)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 3, marginBottom: 1 }}>
                      ...
                      <span style={{ fontSize: 13, color: '#8c8c8c' }}>{item.title}</span>
                    </div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: item.color, lineHeight: 1.3 }}>{item.value}</div>
                    <div style={{ fontSize: 13, color: '#bfbfbf', marginTop: 1, lineHeight: 1.3, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{item.extra}</div>
                  </div>
                ))}
              </div>
              ...
              {activeDifficulty ? (
                <div style={{ padding: '5px 7px', borderRadius: 6, background: '#fff', border: '1px solid rgba(0,0,0,0.06)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      ...
                      <span style={{ fontSize: 13, color: '#595959' }}>难度评估</span>
                    ...
                    <Button icon={<ExperimentOutlined />} loading={difficultyLoading} onClick={handleAiImageAnalysis} disabled={!styleId} style={{ fontSize: 13, height: 20, padding: '0 5px' }}>图像分析</Button>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ fontSize: 13, color: '#595959', whiteSpace: 'nowrap' }}><b>{activeDifficulty.difficultyScore}</b>/10 ×<b style={{ color: '#722ed1' }}>{activeDifficulty.pricingMultiplier}</b></span>
                  </div>
                  <div style={{ fontSize: 13, color: '#8c8c8c', marginTop: 2 }}>BOM {activeDifficulty.bomCount}种 · 工序 {activeDifficulty.processCount}道{activeDifficulty.hasSecondaryProcess ? ' · 含二次工艺' : ''}</div>
                  {activeDifficulty.imageInsight && (() => {
                    ...
                    return (
                      <div style={{ fontSize: 13, color: isError ? '#8c8c8c' : '#595959', marginTop: 3, lineHeight: 1.5, background: isError ? 'rgba(0,0,0,0.02)' : 'rgba(114,46,209,0.03)', borderRadius: 4, padding: '3px 5px' }}>
                        {isError ? '' : ''} {insight}
                      </div>
                    );
                  })()}
                  {visualResult && (
                    <div style={{ marginTop: 4, padding: '4px 6px', borderRadius: 4, background: 'rgba(0,229,255,0.05)', border: '1px solid rgba(0,229,255,0.15)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 3 }}>
                        ...
                        <span style={{ fontSize: 12, color: '#8c8c8c', marginLeft: 'auto' }}>置信度 {Math.round(visualResult.confidence * 100)}%</span>
                      </div>
                      <div style={{ fontSize: 13, color: '#595959', lineHeight: 1.5 }}>{visualResult.summary}</div>
                      {visualResult.defects && visualResult.defects.length > 0 && (
                        <div style={{ marginTop: 3 }}>
                          {visualResult.defects.slice(0, 3).map((d, i) => (
                            <div key={i} style={{ fontSize: 12, color: '#8c8c8c', lineHeight: 1.4 }}>• [{d.level}] {d.type} — {d.description}{d.location ? ` @ ${d.location}` : ''}</div>
                          ))}
                        </div>
                      )}
                ...
                <Button icon={<ExperimentOutlined />} loading={difficultyLoading} onClick={handleAiImageAnalysis} disabled={!styleId} style={{ fontSize: 13 }}>AI 难度分析</Button>
              )}
            </div>
            ...
            <div style={{ flex: 1, minWidth: 0, padding: '6px 8px', borderRadius: 7, background: 'rgba(24,144,255,0.05)', border: '1px solid rgba(24,144,255,0.1)' }}>
              <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 5, color: '#1677ff' }}>AI全链路判断</div>
              {loading ? (
                <Spin />
                    ...
                    return (
                      <div key={item.key} style={{ padding: '5px 7px', borderRadius: 6, ...tone }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 }}>
                          ...
                          <span style={{ padding: '0 5px', borderRadius: 999, background: tone.tagBg, color: tone.tagColor, fontSize: 13, fontWeight: 600 }}>
                            {item.level === 'high' ? '高风险' : item.level === 'medium' ? '需关注' : '已识别'}
                          </span>
                        </div>
                        <div style={{ fontSize: 12, color: '#262626', lineHeight: 1.5 }}>{item.issue}</div>
                        <div style={{ fontSize: 13, color: '#8c8c8c', lineHeight: 1.4, marginTop: 2 }}>{item.cause}</div>
                      </div>
                    );
                  })}
                </div>
