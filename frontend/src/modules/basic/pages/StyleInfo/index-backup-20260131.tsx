...
import { App, Button, Card, Checkbox, Col, Form, Input, InputNumber, Row, Select, Space, Tabs, Tag, Segmented, Statistic, Spin, Modal } from 'antd';
...
import { PlusOutlined, SearchOutlined, EyeOutlined, AppstoreOutlined, UnorderedListOutlined, PrinterOutlined } from '@ant-design/icons';
...
// 自定义 Hooks
import { useStyleList, useStyleStats } from './hooks';
...
const StyleInfoPage: React.FC = () => {
  ...
  const handleDeleteStyle = async (id: string | number) => {
    // 获取款式信息用于确认提示
...
const CoverImageUpload: React.FC<CoverImageUploadProps> = ({
  ...
  const handleDeleteAttachment = async (attachmentId: string | number, localIndex?: number) => {
    // 新建模式下删除本地文件
                  ...
                  {hover && canOperate && (
                    ...
                    >
                      ...
                      <div
                        onClick={(e) => {
                          e.stopPropagation();
                          handleDeleteAttachment(img.id, img.localIndex);
                        }}
                        style={{
                          width: 28,
                          height: 28,
