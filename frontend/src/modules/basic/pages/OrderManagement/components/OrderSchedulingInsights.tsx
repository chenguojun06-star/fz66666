  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {visible ? (
        ...
        >
          {loading ? (
            <div style={{ padding: '10px 0', fontSize: 14, color: '#8c8c8c', textAlign: 'center' }}>正在分析工厂排产...</div>
          ) : items.length === 0 ? (
            <div style={{ padding: '10px 0', fontSize: 14, color: '#8c8c8c', textAlign: 'center', lineHeight: '20px' }}>
              当前没有可用的排产建议数据
              <br />
              不代表不能下单，可继续手动选择工厂
            </div>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 14, color: '#8c8c8c', marginTop: 2 }}>{item.estimatedText}</div>
                  </div>
