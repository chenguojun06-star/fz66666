...
const FactoryOverviewCard: React.FC<any> = ({
  ...
  <div className="c-card" style={{ padding: '12px 14px' }}>
    ...
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginTop: 6 }}>
      ...
      <span style={{ color: 'var(--color-blue-300)', fontSize: 14, fontWeight: 600 }}>工厂</span>
      <span style={{ marginLeft: 'auto', color: 'var(--color-blue-300)', fontSize: 14 }}>
    ...
    <div style={{ display: 'flex', gap: 12, marginTop: 8, flexWrap: 'wrap' }}>
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(255,115,0,0.10)', border: '1px solid rgba(255,115,0,0.3)', borderRadius: 99, padding: '3px 10px', fontSize: 14, color: 'var(--color-orange-500)', fontWeight: 700 }}>
    ...
    {pulse.factoryActivity?.filter((f: any) => f.active).length > 0 && (
      <div style={{ marginTop: 10 }}>
        <div style={{ color: 'var(--color-blue-300)', fontSize: 14, marginBottom: 4 }}>活跃工厂</div>
        {pulse.factoryActivity.filter((f: any) => f.active).map((f: any) => {
          const cap = factoryCapMap[f.factoryName] || {};
          return (
            <div key={f.factoryName} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 14, marginBottom: 3 }}>
              <LiveDot size={5} color="var(--color-accent-neon)" />
              <span style={{ color: 'var(--color-border-light)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.factoryName}</span>
              {cap.orderCount != null && <span style={{ color: 'var(--color-blue-300)', fontSize: 14 }}>{cap.orderCount}单&nbsp;{(cap.totalQuantity||0).toLocaleString()}件</span>}
            </div>
          );
        })}
      </div>
    ...
    {pulse.stagnantFactories?.length > 0 && (
      <div style={{ marginTop: 10, borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 8 }}>
        <div style={{ color: 'var(--color-orange-500)', fontSize: 14, marginBottom: 4 }}>停滞工厂</div>
        {pulse.stagnantFactories.map((sf: any) => {
          const m = sf.minutesSilent || 0;
          const h = Math.floor(m / 60);
          const rm = m % 60;
          return (
            <div key={sf.factoryName} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 14, marginBottom: 3 }}>
              <LiveDot size={5} color="var(--color-orange-500)" />
              <span style={{ color: 'var(--color-border-light)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sf.factoryName}</span>
              <span style={{ color: 'var(--color-orange-500)', fontSize: 14 }}>{h > 0 ? `${h}h${rm}m` : `${rm}m`} 静默</span>
            </div>
          );
        })}
      </div>
    ...
    {factoryCapacity?.length === 0 && pulse.factoryActivity?.length === 0 && (
      <div style={{ color: 'var(--color-blue-400)', fontSize: 14, textAlign: 'center', marginTop: 14 }}>暂无工厂产能数据</div>
    )}
