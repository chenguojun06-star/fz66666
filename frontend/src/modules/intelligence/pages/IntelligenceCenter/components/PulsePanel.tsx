    ...
    <div className="c-card c-scanline-card">
      ...
      <div style={{ overflow: 'hidden', maxHeight: collapsedPanels['pulse'] ? 0 : 800, transition: 'max-height 0.28s ease' }}>
        <div style={{ margin: '6px 0 4px' }}>
          {!isLowEnd && <Sparkline pts={(pulse?.timeline ?? []).map((p: any) => Number(p.quantity) || 0)} color="var(--color-accent-cyan-bright)" width={340} height={52} />}
            ...
            {pulse!.factoryActivity.map((f: any) => {
              ...
              return (
                <div key={f.factoryName} className={`c-factory-activity-row${f.active ? '' : ' inactive'}`}>
                  <span className="c-fa-dot" style={{ background: f.active ? 'var(--color-accent-neon)' : mins < 90 ? 'var(--color-warning-deep)' : 'var(--color-danger)' }} />
                  <span className="c-fa-name">{f.factoryName}</span>
                  <span className="c-fa-time" style={{ color: f.active ? 'var(--color-accent-neon)' : mins < 90 ? 'var(--color-warning-deep)' : 'var(--color-danger)' }}>{timeStr}</span>
