    ...
    <div>
      ...
      <ResizableModal open={modal.visible} title="新建租户" onCancel={modal.close} width="40vw" footer={<Space><Button onClick={modal.close}>取消</Button><Button type="primary" onClick={handleCreate}>确认创建</Button></Space>}>
        <Form form={form} layout="vertical">
          <Form.Item label="租户类型" name="tenantType" initialValue="HYBRID" rules={[{ required: true }]} style={{ marginBottom: 16 }}><Radio.Group style={{ width: '100%' }}><div style={{ display: 'flex', gap: 8 }}><Radio value="SELF_FACTORY" style={{ flex: 1, margin: 0, alignItems: 'flex-start', padding: '8px 10px', border: '1px solid var(--color-border-light)', borderRadius: 6 }}><span style={{ fontWeight: 600, whiteSpace: 'nowrap' }}> 自建工厂</span><div style={{ fontSize: 14, color: 'var(--color-text-muted)', marginTop: 2, lineHeight: 1.3 }}>含裁剪管理，不含外发工厂</div></Radio><Radio value="HYBRID" style={{ flex: 1, margin: 0, alignItems: 'flex-start', padding: '8px 10px', border: '1px solid var(--color-border-light)', borderRadius: 6 }}><span style={{ fontWeight: 600, whiteSpace: 'nowrap' }}> 混合型（推荐）</span><div style={{ fontSize: 14, color: 'var(--color-text-muted)', marginTop: 2, lineHeight: 1.3 }}>自有产线 + 外发合作，全功能</div></Radio><Radio value="BRAND" style={{ flex: 1, margin: 0, alignItems: 'flex-start', padding: '8px 10px', border: '1px solid var(--color-border-light)', borderRadius: 6 }}><span style={{ fontWeight: 600, whiteSpace: 'nowrap' }}> 纯品牌 / 贸易</span><div style={{ fontSize: 14, color: 'var(--color-text-muted)', marginTop: 2, lineHeight: 1.3 }}>全部外发，不含裁剪管理</div></Radio></div></Radio.Group></Form.Item>
      ...
      <ResizableModal open={qrModal.visible} title={`注册二维码 - ${qrModal.data?.tenantName || ''}`} onCancel={qrModal.close} width="40vw" footer={<Button onClick={qrModal.close}>关闭</Button>}>
        {qrModal.data && (<div style={{ textAlign: 'center', padding: '24px 0' }}><div style={{ marginBottom: 20 }}><QRCode value={getRegisterUrl(qrModal.data)} size={240} style={{ margin: '0 auto' }} /></div><div style={{ marginBottom: 16 }}><Text type="secondary">员工扫码或打开链接即可注册到该工厂</Text></div><Card style={{ textAlign: 'left', maxWidth: 400, margin: '0 auto', background: 'var(--color-slate-50)', borderRadius: 8 }}><div style={{ marginBottom: 12 }}><Text strong>工厂名称：</Text><Text>{qrModal.data.tenantName}</Text></div><div style={{ marginBottom: 12 }}><Text strong>工厂编码：</Text><Text code copyable={{ text: qrModal.data.tenantCode }}>{qrModal.data.tenantCode}</Text></div><div style={{ marginBottom: 12 }}><Text strong>注册链接：</Text><div style={{ wordBreak: 'break-all', marginTop: 4 }}><Text type="secondary" style={{ fontSize: 14 }}>{getRegisterUrl(qrModal.data)}</Text></div></div><Space><Button icon={<CopyOutlined />} onClick={() => handleCopyLink(qrModal.data!)}>复制链接</Button><Button icon={<QrcodeOutlined />} onClick={() => handleCopyCode(qrModal.data!.tenantCode)}>复制编码</Button></Space></Card><div style={{ marginTop: 16 }}><Text type="secondary" style={{ fontSize: 14 }}>提示：员工注册后需要管理员在「注册审批」中审批通过后才能使用</Text></div></div>)}
      </ResizableModal>
              ...
              {categories.map(cat => {
                ...
                return (
                  <div key={cat} style={{ marginBottom: 16 }}>
                    <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--color-text-secondary, var(--color-gray-dark))', marginBottom: 8, paddingBottom: 4, borderBottom: '1px solid var(--color-border-light, var(--color-border-light))' }}>{cat}</div>
                    {opts.map(opt => {
