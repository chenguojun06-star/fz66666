    ...
    <div>
      ...
      <ResizableModal open={modal.visible} title="新建租户" onCancel={modal.close} width="40vw" footer={<Space><Button onClick={modal.close}>取消</Button><Button type="primary" onClick={handleCreate}>确认创建</Button></Space>}>
        <Form form={form} layout="vertical">
          <Form.Item label="租户类型" name="tenantType" initialValue="HYBRID" rules={[{ required: true }]} style={{ marginBottom: 16 }}><Radio.Group style={{ width: '100%' }}><div className="u-d-flex u-gap-8"><Radio value="SELF_FACTORY" style={{ flex: 1, margin: 0, alignItems: 'flex-start', padding: '8px 10px', border: '1px solid var(--color-border-light)', borderRadius: 6 }}><span className="u-fw-600 u-ws-nowrap"> 自建工厂</span><div className="u-fs-14 u-mt-2" style={{ color: 'var(--color-text-muted)', lineHeight: 1.3 }}>含裁剪管理，不含外发工厂</div></Radio><Radio value="HYBRID" style={{ flex: 1, margin: 0, alignItems: 'flex-start', padding: '8px 10px', border: '1px solid var(--color-border-light)', borderRadius: 6 }}><span className="u-fw-600 u-ws-nowrap"> 混合型（推荐）</span><div className="u-fs-14 u-mt-2" style={{ color: 'var(--color-text-muted)', lineHeight: 1.3 }}>自有产线 + 外发合作，全功能</div></Radio><Radio value="BRAND" style={{ flex: 1, margin: 0, alignItems: 'flex-start', padding: '8px 10px', border: '1px solid var(--color-border-light)', borderRadius: 6 }}><span className="u-fw-600 u-ws-nowrap"> 纯品牌 / 贸易</span><div className="u-fs-14 u-mt-2" style={{ color: 'var(--color-text-muted)', lineHeight: 1.3 }}>全部外发，不含裁剪管理</div></Radio></div></Radio.Group></Form.Item>
          ...
          <div className="u-d-flex u-gap-12"><Form.Item label="租户名称" name="tenantName" rules={[{ required: true }]} style={{ flex: 1, marginBottom: 12 }}><Input /></Form.Item><Form.Item label="租户编码" name="tenantCode" rules={[{ required: true }]} style={{ flex: 1, marginBottom: 12 }}><Input placeholder="唯一编码，工人注册用" /></Form.Item></div>
