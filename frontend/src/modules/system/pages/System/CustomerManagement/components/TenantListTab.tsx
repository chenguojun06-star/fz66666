    ...
    <div>
      ...
      <ResizableModal open={modal.visible} title="新建租户" onCancel={modal.close} width="40vw" footer={<Space><Button onClick={modal.close}>取消</Button><Button type="primary" onClick={handleCreate}>确认创建</Button></Space>}>
        <Form form={form} layout="vertical">
          <Form.Item label="租户类型" name="tenantType" initialValue="HYBRID" rules={[{ required: true }]} style={{ marginBottom: 16 }}><Radio.Group style={{ width: '100%' }}><div style={{ display: 'flex', gap: 8 }}><Radio value="SELF_FACTORY" style={{ flex: 1, margin: 0, alignItems: 'flex-start', padding: '8px 10px', border: '1px solid #f0f0f0', borderRadius: 6 }}><span style={{ fontWeight: 600, whiteSpace: 'nowrap' }}> 自建工厂</span><div style={{ fontSize: 13, color: '#888', marginTop: 2, lineHeight: 1.3 }}>含裁剪管理，不含外发工厂</div></Radio><Radio value="HYBRID" style={{ flex: 1, margin: 0, alignItems: 'flex-start', padding: '8px 10px', border: '1px solid #f0f0f0', borderRadius: 6 }}><span style={{ fontWeight: 600, whiteSpace: 'nowrap' }}> 混合型（推荐）</span><div style={{ fontSize: 13, color: '#888', marginTop: 2, lineHeight: 1.3 }}>自有产线 + 外发合作，全功能</div></Radio><Radio value="BRAND" style={{ flex: 1, margin: 0, alignItems: 'flex-start', padding: '8px 10px', border: '1px solid #f0f0f0', borderRadius: 6 }}><span style={{ fontWeight: 600, whiteSpace: 'nowrap' }}> 纯品牌 / 贸易</span><div style={{ fontSize: 13, color: '#888', marginTop: 2, lineHeight: 1.3 }}>全部外发，不含裁剪管理</div></Radio></div></Radio.Group></Form.Item>
                      ...
                      return (
                        <div key={opt.value} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 8px', borderRadius: 6, marginBottom: 4, background: willGrant ? 'rgba(82,196,26,0.06)' : willRevoke ? 'rgba(255,77,79,0.06)' : 'transparent', border: willGrant ? '1px solid rgba(82,196,26,0.2)' : willRevoke ? '1px solid rgba(255,77,79,0.2)' : '1px solid transparent', transition: 'all 0.2s' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            {isActive ? <Tag color="green" style={{ margin: 0, fontSize: 13 }}>已开通</Tag> : <Tag style={{ margin: 0, fontSize: 13 }}>未开通</Tag>}
                            {sub?.end_time && isActive && <span style={{ fontSize: 13, color: '#999' }}>到期 {String(sub.end_time).substring(0, 10)}</span>}
                          </div>
                          <Space size={4}>
