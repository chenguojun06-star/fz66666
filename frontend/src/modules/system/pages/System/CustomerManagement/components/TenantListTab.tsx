...
const TenantListTab: React.FC = () => {
  ...
  const columns: ColumnsType<TenantInfo> = [
    ...
    { title: '租户编码', dataIndex: 'tenantCode', width: 110, render: (v: string) => v || <span style={{color:'var(--color-text-quaternary)'}}>待分配</span> },
  ...
  return (
    <div>
      ...
      <SmallModal open={rejectModal.visible} title={`拒绝入驻申请 - ${rejectModal.data?.tenantName || ''}`} onCancel={() => { rejectModal.close(); rejectReasonForm.resetFields(); }} footer={<Space><Button onClick={() => { rejectModal.close(); rejectReasonForm.resetFields(); }}>取消</Button><Button danger type="default" loading={processingId === rejectModal.data?.id} onClick={handleRejectApplication}>确认拒绝</Button></Space>}>
        <Form form={rejectReasonForm} layout="vertical"><Alert title={`申请账号：${rejectModal.data?.applyUsername || '-'}`} type="warning" showIcon className="u-mb-16" /><Form.Item label="拒绝原因" name="reason" rules={[{ required: true, message: '请填写拒绝原因' }]}><Input.TextArea rows={3} placeholder="请填写拒绝原因（将记录在备注中）" /></Form.Item></Form>
      </SmallModal>
      ...
      <ResizableModal open={modal.visible} title="新建租户" onCancel={modal.close} width="40vw" footer={<Space><Button onClick={modal.close}>取消</Button><Button type="primary" onClick={handleCreate}>确认创建</Button></Space>}>
        <Form form={form} layout="vertical">
          <Form.Item label="租户类型" name="tenantType" initialValue="HYBRID" rules={[{ required: true }]} className="u-mb-16"><Radio.Group className="u-w-full"><div className="u-d-flex u-gap-8"><Radio value="SELF_FACTORY" className="u-flex-1 u-m-0 u-ai-start u-p-8px10px u-br-6" style={{ border: '1px solid var(--color-border-light)' }}><span className="u-fw-600 u-ws-nowrap"> 自建工厂</span><div className="u-fs-14 u-mt-2" style={{ color: 'var(--color-text-muted)', lineHeight: 1.3 }}>含裁剪管理，不含外发工厂</div></Radio><Radio value="HYBRID" className="u-flex-1 u-m-0 u-ai-start u-p-8px10px u-br-6" style={{ border: '1px solid var(--color-border-light)' }}><span className="u-fw-600 u-ws-nowrap"> 混合型（推荐）</span><div className="u-fs-14 u-mt-2" style={{ color: 'var(--color-text-muted)', lineHeight: 1.3 }}>自有产线 + 外发合作，全功能</div></Radio><Radio value="BRAND" className="u-flex-1 u-m-0 u-ai-start u-p-8px10px u-br-6" style={{ border: '1px solid var(--color-border-light)' }}><span className="u-fw-600 u-ws-nowrap"> 纯品牌 / 贸易</span><div className="u-fs-14 u-mt-2" style={{ color: 'var(--color-text-muted)', lineHeight: 1.3 }}>全部外发，不含裁剪管理</div></Radio></div></Radio.Group></Form.Item>
          <Form.Item label="初始套餐" name="planType" initialValue="TRIAL" rules={[{ required: true }]} className="u-mb-12"><Select onChange={handleCreatePlanChange}>{PLAN_OPTIONS.map(plan => (<Select.Option key={plan.value} value={plan.value}>{plan.label}（{plan.monthlyFee === 0 ? '免费试用' : `¥${plan.monthlyFee}/月`}，{plan.maxUsers}用户，{plan.storageQuotaMb >= 1024 ? `${plan.storageQuotaMb / 1024}GB` : `${plan.storageQuotaMb}MB`}）</Select.Option>))}</Select></Form.Item>
          <Form.Item noStyle shouldUpdate={(prev, cur) => prev.planType !== cur.planType}>{({ getFieldValue }) => { const plan = PLAN_OPTIONS.find((item) => item.value === getFieldValue('planType')); if (!plan) return null; return <Alert type="info" showIcon className="u-mb-12" title={`当前套餐：${plan.label}`} description={`${plan.monthlyFee === 0 ? '免费试用' : `¥${plan.monthlyFee}/月`} · ${plan.maxUsers} 用户 · ${plan.storageQuotaMb >= 1024 ? `${plan.storageQuotaMb / 1024}GB` : `${plan.storageQuotaMb}MB`} 存储`} />; }}</Form.Item>
          <div className="u-d-flex u-gap-12"><Form.Item label="租户名称" name="tenantName" rules={[{ required: true }]} className="u-flex-1 u-mb-12"><Input /></Form.Item><Form.Item label="租户编码" name="tenantCode" rules={[{ required: true }]} className="u-flex-1 u-mb-12"><Input placeholder="唯一编码，工人注册用" /></Form.Item></div>
          <div className="u-d-flex u-gap-12"><Form.Item label="联系人" name="contactName" rules={[{ required: true }]} className="u-flex-1 u-mb-12"><Input /></Form.Item><Form.Item label="联系电话" name="contactPhone" className="u-flex-1 u-mb-12"><Input /></Form.Item><Form.Item label="最大用户数" name="maxUsers" className="u-mb-12" style={{ flex: '0 0 120px' }}><InputNumber min={1} max={9999} className="u-w-full" disabled /></Form.Item></div>
          <div className="u-br-8 u-mt-8 u-mb-8" style={{ background: 'rgba(82,196,26,0.06)', padding: '12px 16px' }}>
            <div className="u-fw-600 u-mb-8">AI 能力配置（开户即用，租户无需自备Key）</div>
            <Form.Item label="AI配置方式" name="aiConfigMode" initialValue="platform" className="u-mb-8">
              <Radio.Group>
            ...
            <Form.Item noStyle shouldUpdate={(prev, cur) => prev.aiConfigMode !== cur.aiConfigMode}>
              {({ getFieldValue }) => {
                if (getFieldValue('aiConfigMode') === 'tenant') {
                  return <Form.Item label="租户 API Key" name="tenantAiApiKey" className="u-mb-8"><Input.Password placeholder="填入租户自己的 API Key" /></Form.Item>;
                }
                return null;
              }}
            </Form.Item>
            <Alert type="success" showIcon className="u-fs-13"
              title="租户登录后即可使用全部 AI 功能（小云助手、智能巡检、交期预测等），无需自行注册任何 AI 服务平台。"
            />
          </div>
          <div className="u-br-8 u-mt-8" style={{ background: 'rgba(45, 127, 249, 0.08)', padding: '12px 16px' }}><div className="u-fw-600 u-mb-8">主账号信息</div><Form.Item label="用户名" name="ownerUsername" rules={[{ required: true }]}><Input /></Form.Item><Form.Item label="密码" name="ownerPassword" rules={[{ required: true, min: 6 }]}><Input.Password autoComplete="new-password" /></Form.Item><Form.Item label="姓名" name="ownerName"><Input /></Form.Item></div>
        </Form>
      </ResizableModal>
      ...
      <ResizableModal open={qrModal.visible} title={`注册二维码 - ${qrModal.data?.tenantName || ''}`} onCancel={qrModal.close} width="40vw" footer={<Button onClick={qrModal.close}>关闭</Button>}>
        {qrModal.data && (<div className="u-ta-center u-p-24px0"><div style={{ marginBottom: 20 }}><QRCode value={getRegisterUrl(qrModal.data)} size={240} style={{ margin: '0 auto' }} /></div><div className="u-mb-16"><Text type="secondary">员工扫码或打开链接即可注册到该工厂</Text></div><Card className="u-ta-left u-br-8" style={{ maxWidth: 400, margin: '0 auto', background: 'var(--color-slate-50)' }}><div className="u-mb-12"><Text strong>工厂名称：</Text><Text>{qrModal.data.tenantName}</Text></div><div className="u-mb-12"><Text strong>工厂编码：</Text><Text code copyable={{ text: qrModal.data.tenantCode }}>{qrModal.data.tenantCode}</Text></div><div className="u-mb-12"><Text strong>注册链接：</Text><div className="u-mt-4" style={{ wordBreak: 'break-all' }}><Text type="secondary" className="u-fs-14">{getRegisterUrl(qrModal.data)}</Text></div></div><Space><Button icon={<CopyOutlined />} onClick={() => handleCopyLink(qrModal.data!)}>复制链接</Button><Button icon={<QrcodeOutlined />} onClick={() => handleCopyCode(qrModal.data!.tenantCode)}>复制编码</Button></Space></Card><div className="u-mt-16"><Text type="secondary" className="u-fs-14">提示：员工注册后需要管理员在「注册审批」中审批通过后才能使用</Text></div></div>)}
      </ResizableModal>
      ...
      <SmallModal open={resetPwdModal.visible} title={`重置主账号密码 - ${resetPwdModal.data?.tenantName || ''}`} onCancel={() => { resetPwdModal.close(); resetPwdForm.resetFields(); }} footer={<Space><Button onClick={() => { resetPwdModal.close(); resetPwdForm.resetFields(); }}>取消</Button><Button type="default" danger loading={resettingPwd} onClick={handleResetOwnerPassword}>确认重置</Button></Space>}>
        <div className="u-mb-12 u-fs-14" style={{ color: 'var(--color-text-secondary)' }}>主账号：<strong style={{ color: 'var(--primary-color)' }}>{resetPwdModal.data?.ownerUsername || resetPwdModal.data?.applyUsername || '-'}</strong></div>
      ...
      <ResizableModal open={grantModal.visible} title={<><AppstoreOutlined className="u-mr-6" />管理「{grantModal.data?.tenantName || ''}」的应用权限</>} onCancel={grantModal.close} width="85vw" initialHeight={Math.round(window.innerHeight * 0.82)} footer={<Space><Button onClick={grantModal.close}>取消</Button><Button type="primary" loading={granting} onClick={handleGrantModules} disabled={grantAppCodes.length === 0 && revokeAppCodes.length === 0}>{grantAppCodes.length > 0 && revokeAppCodes.length > 0 ? '开通 + 撤销' : grantAppCodes.length > 0 ? '确认开通' : '确认撤销'}</Button></Space>}>
        <Alert title="超管可直接为租户开通或撤销应用商店中的任意应用，无需租户下单付费。API 对接类应用开通后自动生成凭证。" type="info" showIcon className="u-mb-16" />
        {loadingSubs ? <div className="u-ta-center u-p-24" style={{ color: 'var(--color-text-tertiary)' }}>加载订阅状态...</div> : (() => {
          ...
          return (
            <div style={{ maxHeight: 'calc(82vh - 220px)', overflowY: 'auto', paddingRight: 4 }}>
              {categories.map(cat => {
                ...
                return (
                  <div key={cat} className="u-mb-16">
                    <div className="u-fw-600 u-fs-14 u-mb-8" style={{ color: 'var(--color-text-secondary, var(--color-gray-dark))', paddingBottom: 4, borderBottom: '1px solid var(--color-border-light, var(--color-border-light))' }}>{cat}</div>
                    {opts.map(opt => {
                      ...
                      return (
                        <div key={opt.value} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 8px', borderRadius: 6, marginBottom: 4, background: willGrant ? 'rgba(82,196,26,0.06)' : willRevoke ? 'rgba(255,77,79,0.06)' : 'transparent', border: willGrant ? '1px solid rgba(82,196,26,0.2)' : willRevoke ? '1px solid rgba(255,77,79,0.2)' : '1px solid transparent', transition: 'all 0.2s' }}>
                          <div className="u-d-flex u-ai-center u-gap-8">
                            ...
                            {isActive ? <Tag color="green" className="u-m-0 u-fs-14">已开通</Tag> : <Tag className="u-m-0 u-fs-14">未开通</Tag>}
        ...
        {grantAppCodes.length > 0 && (
          <div className="u-mt-16 u-br-8" style={{ padding: '10px 12px', background: 'rgba(82,196,26,0.06)', border: '1px solid rgba(82,196,26,0.15)' }}>
            <div className="u-fw-600 u-mb-8">开通有效期：</div>
        ...
        {(grantAppCodes.length > 0 || revokeAppCodes.length > 0) && (
          <div className="u-mt-12 u-fs-14" style={{ color: 'var(--color-text-tertiary)' }}>
