...
const FlowCard: React.FC<{ icon: React.ReactNode; title: string; desc: string; color: string }> = ({ icon, title, desc, color }) => (
  <Card style={{ borderLeft: `4px solid ${color}`, height: '100%' }} styles={{ body: { padding: '12px 16px' } }}>
    ...
    <Text type="secondary" className="u-fs-14">{desc}</Text>
  </Card>
);

const IntegrationGuideTab: React.FC = () => (
  <div style={{ maxWidth: 1100, margin: '0 auto' }}>
    <div className="u-ta-center" style={{ padding: '24px 0 16px' }}>
      <Title level={3} className="u-mb-4"><BookOutlined className="u-mr-8" style={{ color: 'var(--color-primary)' }} />API对接使用教程</Title>
      <Text type="secondary">一站式了解如何购买、配置和使用API对接功能，与您的系统无缝集成</Text>
    </div>

    <IntegrationQuickStart />

    <Card title={<span><SwapOutlined className="u-mr-8" style={{ color: 'var(--color-info)' }} />五大对接模块</span>} className="u-mb-24">
      <Row gutter={[16, 16]}>
      ...
      <div className="u-br-8 u-fs-14 u-ov-auto" style={{ background: 'var(--color-bg-stripe)', padding: 20, fontFamily: 'monospace', lineHeight: 2 }}>
        <div className="u-d-grid u-ai-center" style={{ gridTemplateColumns: '1fr auto 1fr', gap: '4px 12px', maxWidth: 700 }}>
    ...
    <Card title={<span><LockOutlined className="u-mr-8" style={{ color: 'var(--color-warning)' }} />API认证签名</span>} className="u-mb-24">
      <Alert type="info" showIcon title="所有API请求必须携带签名，签名算法为 HMAC-SHA256，使用 appSecret 作为密钥。" className="u-mb-16" />
      <Collapse items={[
    ...
    <Card title={<span><CodeOutlined className="u-mr-8" style={{ color: 'var(--color-success)' }} />API接口参考</span>} className="u-mb-24">
      <Tabs items={[
        ...
        { key: 'payment', label: '付款对接', children: (<div><Title level={5}>Webhook: payment_sync</Title><CopyBlock lang="json" code={`{\n  "event": "payment_sync",\n  "reconciliationNo": "MR20260310001",\n  "amount": 15000.00,\n  "status": "approved",\n  "timestamp": "2026-03-10T17:00:00Z"\n}`} /><Title level={5} className="u-mt-16">POST /openapi/v1/payment/confirm — 确认付款</Title></div>) },
        { key: 'material', label: '面辅料供应', children: (<div><Title level={5}>POST /openapi/v1/material/purchase-order — 推送采购订单</Title><Title level={5} className="u-mt-16">GET /openapi/v1/material/stock — 查询供应商库存</Title><Title level={5} className="u-mt-16">Webhook: material_supply_update</Title></div>) },
      ]} />
    </Card>

    <Card title={<span><QuestionCircleOutlined className="u-mr-8" style={{ color: 'var(--color-warning)' }} />常见问题</span>} className="u-mb-24">
      <Collapse items={[
