...
const CopyBlock: React.FC<{ code: string; lang?: string }> = ({ code, lang = '' }) => {
  ...
  return (
    <div className="u-pos-relative u-br-8 u-m-12px0 u-ov-auto" style={{ background: 'var(--color-bg-base)', padding: '16px 48px 16px 16px' }}>
      <Button icon={<CopyOutlined />} type="text" className="u-pos-absolute" style={{ top: 8, right: 8, color: 'var(--color-text-quaternary)' }} onClick={handleCopy} />
      {lang && <Tag className="u-pos-absolute" style={{ top: 8, left: 12, opacity: 0.7 }}>{lang}</Tag>}
