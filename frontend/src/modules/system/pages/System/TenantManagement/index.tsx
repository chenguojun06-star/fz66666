...
import { Tabs, Table, Button, Tag, Space, message, Form, Input, InputNumber, Modal, Select, Card, Statistic, Row, Col, Typography, Descriptions, Badge, Tooltip, Timeline, Empty } from 'antd';
import { PlusOutlined, TeamOutlined, CrownOutlined, SafetyCertificateOutlined, ApiOutlined, CopyOutlined, StopOutlined, PlayCircleOutlined, CodeOutlined, DashboardOutlined, LinkOutlined, CheckCircleOutlined, SwapOutlined, EyeOutlined, BookOutlined } from '@ant-design/icons';
      ...
      <Row gutter={16} style={{ marginBottom: 24 }}>
        ...
        <Col span={6}>
          <Card size="small" style={{ borderLeft: '3px solid var(--color-success)' }}>
            <Statistic title="运行中" value={overview?.activeApps || 0} suffix="个" valueStyle={{ color: 'var(--color-success)' }} prefix={<CheckCircleOutlined />} />
          </Card>
        </Col>
        <Col span={6}>
  ...
  return (
    <div>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        ...
        <Col span={6}><Card size="small"><Statistic title="运行中" value={stats.active} valueStyle={{ color: 'var(--color-success)' }} prefix={<PlayCircleOutlined />} /></Card></Col>
        <Col span={6}><Card size="small"><Statistic title="已停用" value={stats.disabled} valueStyle={{ color: 'var(--color-danger)' }} prefix={<StopOutlined />} /></Card></Col>
...
const TenantListTab: React.FC = () => {
  ...
  const columns: ColumnsType<TenantInfo> = [
    { title: '租户名称', dataIndex: 'tenantName', width: 160 },
    { title: '租户编码', dataIndex: 'tenantCode', width: 120 },
    { title: '联系人', dataIndex: 'contactName', width: 100 },
    { title: '联系电话', dataIndex: 'contactPhone', width: 130 },
    { title: '最大用户数', dataIndex: 'maxUsers', width: 100, align: 'center' },
    {
      title: '状态', dataIndex: 'status', width: 80, align: 'center',
      render: (s: string) => <Tag color={s === 'active' ? 'green' : 'red'}>{s === 'active' ? '正常' : '停用'}</Tag>,
    },
    { title: '创建时间', dataIndex: 'createTime', width: 160 },
    {
      title: '操作', key: 'actions', width: 160,
    ...
    <Layout>
        <Tabs
          ...
          items={[
            ...
            ...(isSuperAdmin ? [{
              key: 'tenants',
              label: <span><CrownOutlined /> 租户管理</span>,
