...
export const APP_TYPE_CONFIG: Record<string, { label: string; color: string; icon: string; description: string }> = {
  ...
  MATERIAL_SUPPLY:  { label: '面辅料供应对接', color: 'var(--color-accent-cyan)',              icon: '', description: '面辅料供应商系统直接同步采购、入库数据' },
  DATA_IMPORT:      { label: '数据导入',       color: 'var(--color-accent-purple)',              icon: '', description: '批量导入生产订单、工序、库存等数据，开通即用' },
  EC_TAOBAO:        { label: '淘宝对接',       color: 'var(--color-orange-500)',              icon: '', description: '淘宝平台订单自动同步到生产系统' },
  EC_TMALL:         { label: '天猫对接',       color: 'var(--color-red-500)',              icon: '', description: '天猫旗舰店订单实时同步，支持商品编码映射' },
  EC_JD:            { label: '京东对接',       color: 'var(--color-red-600)',              icon: '', description: '京东平台订单自动同步，支持物流回传' },
  EC_DOUYIN:        { label: '抖音对接',       color: 'var(--color-black)',              icon: '', description: '抖音小店订单实时接入，直播售卖自动下单' },
  EC_PINDUODUO:     { label: '拼多多对接',     color: 'var(--color-red-600)',              icon: '', description: '拼多多平台订单同步，支持多规格商品编码' },
  EC_XIAOHONGSHU:   { label: '小红书对接',     color: 'var(--color-rose-500)',              icon: '', description: '小红书商城订单同步，种草转化直连生产' },
  EC_WECHAT_SHOP:   { label: '微信小店对接',   color: 'var(--color-emerald-500)',              icon: '', description: '微信小店/视频号订单直接推送生产系统' },
  EC_SHOPIFY:       { label: 'Shopify对接',    color: 'var(--color-lime-500)',              icon: '', description: '跨境Shopify店铺订单自动同步，支持多货币' },
};
export const MODULE_ICONS: Record<string, { icon: string; color: string; bgColor: string }> = {
  ...
  MATERIAL_SUPPLY:  { icon: '', color: 'var(--color-accent-cyan)',              bgColor: 'rgba(19, 194, 194, 0.1)' },
  DATA_IMPORT:      { icon: '', color: 'var(--color-accent-purple)',              bgColor: 'rgba(114, 46, 209, 0.1)' },
  EC_TAOBAO:        { icon: '', color: 'var(--color-orange-500)',              bgColor: 'rgba(255, 69, 0, 0.1)' },
  EC_TMALL:         { icon: '', color: 'var(--color-red-500)',              bgColor: 'rgba(255, 45, 45, 0.1)' },
  EC_JD:            { icon: '', color: 'var(--color-red-600)',              bgColor: 'rgba(225, 37, 27, 0.1)' },
  EC_DOUYIN:        { icon: '', color: 'var(--color-text-primary)',              bgColor: 'rgba(0,0,0,0.06)' },
  EC_PINDUODUO:     { icon: '', color: 'var(--color-red-600)',              bgColor: 'rgba(224, 46, 36, 0.1)' },
  EC_XIAOHONGSHU:   { icon: '', color: 'var(--color-rose-500)',              bgColor: 'rgba(254, 44, 85, 0.1)' },
  EC_WECHAT_SHOP:   { icon: '', color: 'var(--color-emerald-500)',              bgColor: 'rgba(7, 193, 96, 0.1)' },
  EC_SHOPIFY:       { icon: '', color: 'var(--color-lime-500)',              bgColor: 'rgba(150, 191, 72, 0.1)' },
};
...
export const getFlowDescription = (appType: string): string => {
  const map: Record<string, string> = {
    ...
    LOGISTICS_SYNC: '出库发货 → Webhook推送物流信息 → 在「仓库管理→成品仓库」查看',
