...
export const tutorialsWarehouse: Tutorial[] = [
    {
      ...
      duration: '8分钟',
      tags: ['面辅料', '库存', '出入库', '入库', '出库'],
      steps: [
        {
          title: '进入面辅料出入库页面',
          description: '从左侧菜单选择「物料管理」→「物料出入库」进入。页面顶部显示四个统计卡片：库存总值、总数量、低库存预警数、物料种类数。',
          tips: [
            '此页面管理面辅料的所有进出库操作',
      ...
      faqs: [
        {
          question: '面辅料出入库和物料资料库有什么区别？',
          answer: ' 两个模块职责不同：\n\n物料资料库（采购管理→物料资料库）：\n• 管理物料的"基础信息"：编号、名称、规格、成分、供应商\n• 相当于物料档案库\n• 不涉及库存数量\n\n面辅料出入库（采购管理→面辅料出入库）：\n• 管理物料的"库存数量"：入库、出库、库存查询\n• 实时追踪每种物料有多少\n• 包含预警和统计功能\n\n关系：先在"物料资料库"创建物料档案 → 再在"出入库"进行入库操作',
        },
        {
    ...
    },
    {
      ...
      steps: [
        {
          title: '进入成品出入库页面',
          description: '从左侧菜单选择「成品管理」→「成品出入库」进入。页面以订单+款号维度聚合显示成品库存，同一订单同一款号的不同颜色/尺码自动合并为一行。',
          tips: [
        ...
        {
          title: '成品出库（发货）',
          description: '点击「出库」按钮，弹出商品编码明细出库面板。系统展开该订单的所有颜色×尺码组合，为每个商品编码分别设置出库数量。确认后库存自动扣减。',
          tips: [
            '出库时按商品编码（颜色+尺码）逐个设置数量，精确到件',
            '出库数量不能超过该商品编码的可用库存',
            '支持部分出库（如只发部分颜色/尺码）',
      ...
      faqs: [
        {
          ...
          answer: ' 两个操作在不同页面：\n\n成品入库：\n• 页面位置：【生产管理】→【质检入库】\n• 操作：生产完成 → 质检合格 → 录入入库数量\n• 入库后自动显示在成品出入库中\n\n成品出库：\n• 页面位置：【仓库管理】→【成品出入库】\n• 操作：在成品列表中点击【出库】按钮\n• 按商品编码明细设置出库数量 → 确认\n\n 注意：成品出入库页面只能出库，不能直接入库。入库必须通过质检入库流程。',
        },
        {
