...
const AppStore: React.FC = () => {
              ...
              <div className="app-desc">
                ...
                {activated && myApp ? (
                  <div style={{ marginTop: 6 }}>
                    <Tag color={myApp.configured ? 'green' : 'orange'} style={{ fontSize: 14 }}>{myApp.configured ? ' 已配置' : ' 待配置URL'}</Tag>
                    {(myApp.totalCalls ?? 0) > 0 && <Tag style={{ fontSize: 14 }}>调用 {myApp.totalCalls} 次</Tag>}
                  </div>
                ) : (<>
              ...
              <Col xs={24} sm={12} md={6} key={app.appCode}>
                <Card hoverable style={{ borderTop: `3px solid ${cfg.color}`, background: app.isExpired ? '#fafafa' : '#fff' }} onClick={() => navigate('/system/tenant?tab=apps')}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    ...
                    <div><div style={{ fontWeight: 600, fontSize: 14 }}>{app.appName}</div><Tag color={app.isExpired ? 'default' : app.configured ? 'success' : 'warning'} style={{ fontSize: 14 }}>{app.isExpired ? '已过期' : app.configured ? ' 运行中' : ' 待配置'}</Tag></div>
                  </div>
                  {app.appKey && <div style={{ fontSize: 14, color: 'var(--color-text-tertiary)', fontFamily: 'monospace' }}>Key: {app.appKey.substring(0, 16)}...</div>}
                  {app.totalCalls != null && <div style={{ fontSize: 14, color: 'var(--color-text-secondary)', marginTop: 2 }}>累计调用 {app.totalCalls} 次{app.dailyQuota ? ` · 今日 ${app.dailyUsed || 0}/${app.dailyQuota}` : ''}</div>}
                </Card>
              </Col>
            );
          })}
  ...
  return (<>
    <div className="app-store-container">
      ...
      {/* 应用详情弹窗 */}
      <ResizableModal title={<div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 15 }}>{selectedApp?.appIcon}</span><span style={{ fontSize: 15 }}>{selectedApp?.appName}</span>{isAppActivated(selectedApp?.appCode || '') && <Tag color="green">已开通</Tag>}</div>}
        ...
        <div style={{ padding: '8px 0' }}>
          <Alert type="success" showIcon icon={<RocketOutlined />} style={{ marginBottom: 12, fontSize: 14 }} title={<span style={{ fontSize: 14 }}><strong>智能对接：</strong>开通后系统自动生成API凭证，您只需填写您的接口地址即可使用</span>} />
          <div style={{ marginBottom: 12 }}><div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4, borderLeft: '3px solid var(--primary-color, #1890ff)', paddingLeft: 8 }}>应用简介</div><p style={{ fontSize: 14, color: 'var(--color-text-secondary)', margin: 0, lineHeight: 1.6 }}>{selectedApp?.appDesc}</p></div>
          {selectedApp && MODULE_CONFIG[selectedApp.appCode] && (
            <div style={{ marginBottom: 12 }}><div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4, borderLeft: '3px solid var(--primary-color, #1890ff)', paddingLeft: 8 }}>开通后自动获得</div>
          ...
          {parseFeatures(selectedApp?.features).length > 0 && (
            <div style={{ marginBottom: 12 }}><div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4, borderLeft: '3px solid var(--primary-color, #1890ff)', paddingLeft: 8 }}>核心功能</div>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>{parseFeatures(selectedApp?.features).map((f: string, i: number) => (<li key={i} style={{ padding: '2px 0', fontSize: 14 }}><CheckCircleOutlined style={{ color: 'var(--color-success)', marginRight: 6, fontSize: 12 }} />{f}</li>))}</ul></div>
          )}
          <div>
            <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 10, borderLeft: '3px solid var(--primary-color, #1890ff)', paddingLeft: 8 }}>价格方案</div>
            <div style={{ display: 'grid', gridTemplateColumns: selectedApp?.trialDays ? 'repeat(4, 1fr)' : 'repeat(3, 1fr)', gap: 10 }}>
              {selectedApp?.trialDays ? (<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '12px 6px', borderRadius: 10, border: '1.5px solid #52c41a', background: 'linear-gradient(135deg, #f6ffed 0%, #d9f7be 100%)', minHeight: 88 }}><GiftOutlined style={{ fontSize: 15, color: '#52c41a', marginBottom: 4 }} /><div style={{ fontSize: 14, color: '#389e0d', fontWeight: 600, marginBottom: 3 }}>免费试用</div><div style={{ fontSize: 13, fontWeight: 700, color: '#52c41a', lineHeight: 1.2 }}>¥0</div><div style={{ fontSize: 14, color: '#73d13d', marginTop: 3 }}>{selectedApp.trialDays} 天</div></div>) : null}
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '12px 6px', borderRadius: 10, border: '1.5px solid var(--color-border-antd)', background: 'var(--color-bg-container)', minHeight: 88 }}><div style={{ fontSize: 14, color: 'var(--color-text-tertiary)', fontWeight: 500, marginBottom: 4 }}>月 付</div><div style={{ fontSize: 13, fontWeight: 700, color: '#1677ff', lineHeight: 1.2 }}>¥{selectedApp?.priceMonthly}</div><div style={{ fontSize: 14, color: 'var(--color-text-quaternary)', marginTop: 3 }}>/ 月</div></div>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '12px 6px', borderRadius: 10, border: '1.5px solid #faad14', background: 'linear-gradient(135deg, #fffbe6 0%, #fff1b8 100%)', minHeight: 88, position: 'relative' }}><div style={{ position: 'absolute', top: -1, right: -1, background: '#faad14', color: '#fff', fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: '0 9px 0 8px', letterSpacing: 0.5 }}>推荐</div><div style={{ fontSize: 14, color: '#d48806', fontWeight: 600, marginBottom: 4 }}>年 付</div><div style={{ fontSize: 13, fontWeight: 700, color: '#fa8c16', lineHeight: 1.2 }}>¥{selectedApp?.priceYearly}</div><div style={{ fontSize: 9, color: '#ffc53d', marginTop: 3 }}>/ 年 · 省2个月</div></div>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '12px 6px', borderRadius: 10, border: '1.5px solid #adc6ff', background: 'linear-gradient(135deg, #f0f5ff 0%, #d6e4ff 100%)', minHeight: 88 }}><div style={{ fontSize: 14, color: '#2f54eb', fontWeight: 600, marginBottom: 4 }}>买 断</div><div style={{ fontSize: 13, fontWeight: 700, color: '#2f54eb', lineHeight: 1.2 }}>¥{selectedApp?.priceOnce}</div><div style={{ fontSize: 14, color: '#85a5ff', marginTop: 3 }}>永久使用</div></div>
            </div>
          </div>
        </div>
      </ResizableModal>
        ...
        {wizardStep === 0 && (<div>
          {wizardData.appKey && wizardData.appSecret ? (<>
            ...
            <div style={{ background: '#f6f8fa', padding: 16, borderRadius: 8, marginBottom: 16 }}>
              ...
              <div><Text type="secondary" style={{ fontSize: 14 }}>AppSecret</Text><Alert type="warning" showIcon style={{ padding: '4px 8px', fontSize: 14, marginBottom: 4 }} title=" 密镂仅显示一次，请立即保存！" /><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Text code style={{ fontSize: 14, fontWeight: 600, color: '#cf1322' }}>{wizardData.appSecret}</Text><CopyOutlined style={{ cursor: 'pointer', color: 'var(--color-primary)' }} onClick={() => copyToClipboard(wizardData.appSecret || '')} /></div></div>
            </div></>
        ...
        {wizardStep === 1 && (<div>
          {isEcApp(wizardData.appCode || '') ? (<>
            ...
            <Form form={setupForm} layout="vertical">
              ...
              {EC_PLATFORM_MAP[wizardData.appCode || '']?.extraHint && <Form.Item label={<span>扩展字段<Text type="secondary" style={{ fontSize: 14, marginLeft: 4 }}>{EC_PLATFORM_MAP[wizardData.appCode || '']?.extraHint}</Text></span>} name="extraField"><Input placeholder={EC_PLATFORM_MAP[wizardData.appCode || '']?.extraHint} /></Form.Item>}
              <Form.Item label={<span>物流回传地址<Text type="secondary" style={{ fontSize: 14, marginLeft: 4 }}>出库后自动回传物流信息到此地址</Text></span>} name="callbackUrl" rules={[{ type: 'url', message: '请输入正确的URL地址' }]}><Input placeholder="https://open.platform.com/api/logistics/callback" prefix={<LinkOutlined />} /></Form.Item>
            </Form></>
          ) : (<>
            ...
            <Form form={setupForm} layout="vertical">
              <Form.Item label={<span>回调地址（Webhook）<Text type="secondary" style={{ fontSize: 14, marginLeft: 4 }}>我们会向此地址推送数据</Text></span>} name="callbackUrl" rules={[{ type: 'url', message: '请输入正确的URL地址' }]}><Input placeholder={MODULE_CONFIG[wizardData.appCode || '']?.urlHint || 'https://your-system.com/webhook/callback'} prefix={<LinkOutlined />} /></Form.Item>
      ...
      {/* 购买意向弹窗 */}
      <ResizableModal title={<div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 14 }}>{selectedApp?.appIcon}</span><span>购买意向 - {selectedApp?.appName}</span></div>}
        ...
        <div style={{ padding: '8px 0' }}>
          <Alert type="info" showIcon style={{ marginBottom: 12, fontSize: 14 }} title="提交后，商务团队将在1-3个工作日内联系您确认并完成开通。" />
