...
import { Button, Table, Tag, InputNumber, Modal, Input, message, Tooltip, Space, Divider, Empty } from 'antd';
import {
  ShopOutlined,
  SendOutlined,
  UndoOutlined,
    ...
    Modal.confirm({
      ...
      content: (
        <div>
          ...
          <p style={{ color: remainQty > 0 ? '#faad14' : '#52c41a', fontWeight: 600 }}>{confirmMsg}</p>
        </div>
      ),
      okText: '确认领取',
      cancelText: '取消',
    ...
    () => [
      ...
      },
      {
        ...
        render: (stock: number, record: MaterialItem) => {
          ...
          const color = stock <= 0 ? '#ff4d4f' : stock < record.requiredQty ? '#faad14' : '#52c41a';
        ...
        cards={[
          {
            ...
            items: [
              ...
              { label: '待处理', value: pendingCount, unit: '项', color: pendingCount > 0 ? '#faad14' : '#52c41a' },
            ],
          },
          {
            key: 'stock',
            ...
            activeBg: stockStatusColor === '#52c41a' ? '#f6ffed' : stockStatusColor === '#ff4d4f' ? '#fff2f0' : '#fffbe6',
          },
          {
            key: 'picking',
