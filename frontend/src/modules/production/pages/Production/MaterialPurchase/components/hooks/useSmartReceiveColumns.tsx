...
export const useSmartReceiveColumns = ({
  ...
  const materialColumns = useMemo(() => [
    ...
    { title: '领取数量', key: 'pickQty', width: 110, align: 'center' as const, render: (_: unknown, record: MaterialItem) => { if (record.purchaseStatus !== 'pending') return <span style={{ color: 'var(--color-text-tertiary)' }}>-</span>; const maxQty = record.availableStock > 0 ? Math.min(record.availableStock, record.requiredQty) : 0; return <InputNumber min={0} max={maxQty} value={record.userPickQty} onChange={(v) => updatePickQty(record.purchaseId, v)} style={{ width: 80 }} disabled={record.availableStock <= 0} placeholder={record.availableStock <= 0 ? '无库存' : '填写'} />; } },
