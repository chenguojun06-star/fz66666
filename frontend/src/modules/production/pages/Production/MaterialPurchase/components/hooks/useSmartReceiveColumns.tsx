...
export const useSmartReceiveColumns = ({
  ...
  const materialColumns = useMemo(() => [
    ...
    { title: '物料名称', dataIndex: 'materialName', key: 'materialName', width: 140, render: (text: string, record: MaterialItem) => (<div><div style={{ fontWeight: 500 }}>{text || '无'}</div>{record.materialType && <Tag color={getMaterialTypeColor(record.materialType)} style={{ fontSize: 13, marginTop: 2 }}>{getMaterialTypeName(record.materialType)}</Tag>}</div>) },
    ...
    { title: '操作', key: 'actions', width: 180, render: (_: unknown, record: MaterialItem) => { if (record.purchaseStatus === MATERIAL_PURCHASE_STATUS.WAREHOUSE_PENDING) return <span style={{ color: 'var(--color-primary)', fontSize: 12 }}>⏳ 待仓库出库确认</span>; if (record.purchaseStatus !== 'pending') return <span style={{ color: 'var(--color-text-tertiary)', fontSize: 12 }}>已处理</span>; const userPick = record.userPickQty || 0; const remainQty = record.requiredQty - userPick; if (record.availableStock <= 0) return <Button type="primary" ghost icon={<SendOutlined />} loading={actionLoading[record.purchaseId]} onClick={() => handlePurchaseOnly(record)}>一键采购</Button>; return <Space size={4}><Tooltip title={userPick <= 0 ? '请先填写领取数量' : `领取 ${userPick}${remainQty > 0 ? `，差额 ${remainQty} 自动采购` : ''}`}><Button type="primary" icon={<ShopOutlined />} disabled={userPick <= 0} loading={actionLoading[record.purchaseId]} onClick={() => handleWarehousePick(record)}>仓库领取</Button></Tooltip></Space>; } },
  ...
  const pickingColumns = useMemo(() => [
    ...
    ...(isSupervisorOrAbove ? [{ title: '操作', key: 'actions', width: 80, render: (_: unknown, record: PickingRecord) => record.status !== 'cancelled' ? <Button type="link" danger icon={<UndoOutlined />} onClick={() => handleCancelPicking(record)}>撤销</Button> : <span style={{ color: 'var(--color-text-tertiary)', fontSize: 12 }}>已撤销</span> }] : []),
