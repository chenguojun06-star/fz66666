...
export const useSmartReceiveActions = (
  ...
  const handleForcePurchaseAll = (materials: MaterialItem[]) => {
    ...
    Modal.confirm({
      ...
      content: (<div>{hasStockCount > 0 && <p style={{ color: '#fa8c16', fontWeight: 600 }}>有 {hasStockCount} 项物料存在可用库存，确认后将跳过仓库直接外采。</p>}<p>以下 <strong>{allPendingItems.length}</strong> 项物料将全部标记为"采购中"：</p><div style={{ maxHeight: 200, overflow: 'auto', margin: '8px 0', padding: '8px 12px', background: 'var(--color-bg-container)', borderRadius: 4 }}>{allPendingItems.map((item) => (<div key={item.purchaseId} style={{ fontSize: 14, padding: '2px 0' }}>• {item.materialName}（{item.materialCode}）— {item.requiredQty} {item.unit}{item.availableStock > 0 && <span style={{ color: '#fa8c16', marginLeft: 4, fontSize: 14 }}>有库存</span>}</div>))}</div><Divider style={{ margin: '8px 0' }} /><p style={{ color: '#fa8c16', fontWeight: 600 }}>确认后请联系供应商进行采购，不使用仓库库存。</p></div>),
