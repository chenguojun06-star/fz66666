...
const calculateDurationLabel = (record: FlowStage): React.ReactNode => {
  ...
  const color = hours > 336 ? 'var(--color-error)' : hours > 168 ? 'var(--color-warning)' : 'var(--color-gray-700)';
  return React.createElement(
    'span',
    { style: { color, fontSize: 15, fontWeight: hours > 168 ? 600 : 400 } },
...
export const buildOrderLineColumns = (): ColumnsType<OrderLine> => [
  { title: '商品编码号', dataIndex: 'skuNo', key: 'skuNo', width: 240, ellipsis: true, render: (v: unknown) => String(v || '').trim() || '-' },
