...
export function useProgressTracking(productionList: ProductionOrder[]) {
  ...
  const renderCompletionTimeTag = (
    ...
    if (!formatted) {
      return (
        <div
          style={{ fontSize: 12, color: '#d1d5db', lineHeight: 1.2, marginBottom: 1, textAlign: align }}
    ...
    return (
      <div
        style={{ fontSize: 12, color: isComplete ? '#10b981' : '#6b7280', fontWeight: isComplete ? 600 : 400, lineHeight: 1.2, marginBottom: 1, textAlign: align, whiteSpace: 'nowrap' }}
        onMouseEnter={triggerPredictOnHover}
