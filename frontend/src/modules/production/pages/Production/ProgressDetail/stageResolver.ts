...
export const canonicalStageKey = (k: string) => {
  ...
  const map: Record<string, string> = {
    订单创建: '下单', 创建订单: '下单', 开单: '下单', 制单: '下单',
    物料采购: '采购', 面辅料采购: '采购', 备料: '采购', 到料: '采购', 进料: '采购', 物料: '采购',
    缝制: '车缝', 缝纫: '车缝', 车工: '车缝', 整件: '车缝', 生产: '车缝', 制作: '车缝', 车间生产: '车缝',
    裁床: '裁剪', 剪裁: '裁剪', 开裁: '裁剪', 裁片: '裁剪', 裁切: '裁剪',
    绣花: '二次工艺', 印花: '二次工艺', 水洗: '二次工艺', 压花: '二次工艺', 烫钻: '二次工艺',
    烫画: '二次工艺', 钉珠: '二次工艺', 烫金: '二次工艺', 数码印: '二次工艺', 打孔: '二次工艺',
    激光: '二次工艺', 转印: '二次工艺', 植绒: '二次工艺', 涂层: '二次工艺', 磨毛: '二次工艺',
    染色: '二次工艺', 后处理: '二次工艺',
    大烫: '尾部', 整烫: '尾部', 熨烫: '尾部', 烫整: '尾部', 后整烫: '尾部', 剪线: '尾部',
    锁边: '尾部', 检验: '尾部', 品检: '尾部', 验货: '尾部', QC: '尾部', 品控: '尾部', 检查: '尾部',
    后整: '尾部', 打包: '尾部', 装箱: '尾部', 封箱: '尾部', 贴标: '尾部',
    质检入库: '入库', 入仓: '入库', 仓库: '入库', 仓储: '入库', 上架: '入库', 进仓: '入库',
    入库质检: '入库', 成品入库: '入库', 完工入库: '入库',
    发货: '出货', 发运: '出货',
  };
...
export const getOrderStageCompletionTimeFallback = (
  ...
  if (matchAny(isCuttingStageKey)) return pickField('cuttingEndTime');
  if (matchAny(isSecondaryProcessStageKey)) return pickField('secondaryProcessEndTime');
  if (matchAny(isSewingStageKey)) return pickField('carSewingEndTime', 'sewingEndTime');
  if (matchAny(isTailStageKey)) return pickField('packagingEndTime', 'ironingEndTime');
  if (matchAny(isPackagingStageKey)) return pickField('packagingEndTime', 'ironingEndTime');
  if (matchAny(isIroningStageKey)) return pickField('ironingEndTime', 'packagingEndTime');
  if (matchAny(isQualityStageKey)) return pickField('qualityEndTime');
  if (matchAny(isWarehouseStageKey)) return pickField('warehousingEndTime');
...
export const findPricingProcessForStage = (list: StyleProcess[], stageName: string) => {
  ...
  for (const p of sorted) {
    ...
    if (!name) continue;
    if (stageNameMatches(stage, name)) return p;
  }
  return null;
};
