  ...
  return useMemo(() => {
    ...
    for (const n of nodes || []) {
      ...
      const doneFromScans = records
        .filter((r) => stageNameMatches(nodeName, getRecordStageName(r as unknown as Record<string, unknown>)))
        .reduce((acc, r) => acc + (Number((r as unknown as Record<string, unknown>)?.quantity) || 0), 0);
