...
export const getNodeColor = (expectedShipDate: any, isColor2 = false): string => {
  if (!expectedShipDate) return isColor2 ? 'var(--color-success)' : 'var(--color-success)';
  const now = new Date();
  const delivery = new Date(expectedShipDate as string);
  const diffDays = Math.ceil((delivery.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
  if (diffDays < 0) return isColor2 ? 'var(--color-danger)' : 'var(--color-danger)';
  if (diffDays <= 3) return isColor2 ? 'var(--color-amber-300)' : 'var(--color-warning)';
  return isColor2 ? 'var(--color-success)' : 'var(--color-success)';
};
