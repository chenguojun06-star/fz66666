...
type UseFocusNodeEffectParams = {
  ...
  openNodeDetail: (
    order: ProductionOrder, type: string, name: string,
    stats: { done: number; total: number; percent: number; remaining: number },
    unitPrice?: number, processList?: any[],
  ) => void;
  setPendingFocusNode: (node: null) => void;
};

export const useFocusNodeEffect = ({
  pendingFocusNode, orders, progressNodesByStyleNo,
  boardStatsByOrder, boardStatsLoadingByOrder,
  normalizeFocusNodeName, getFocusNodeType,
  triggerOrderFocus, openNodeDetail, setPendingFocusNode,
}: UseFocusNodeEffectParams) => {
  useEffect(() => {
