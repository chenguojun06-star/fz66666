...
export function useCardViewConfig({
  ...
  const titleTags = useCallback((record: any) => (
    <>
      {(record as ProductionOrder).urgencyLevel === 'urgent' && <Tag color="red" className="u-m-0 u-fs-14 u-lh-16px" style={{ padding: '0 3px', height: 16 }}>急</Tag>}
      {String((record as any).plateType || '').toUpperCase() === 'FIRST' && <Tag color="blue" className="u-m-0 u-fs-14 u-lh-16px" style={{ padding: '0 3px', height: 16 }}>首单</Tag>}
      {String((record as any).plateType || '').toUpperCase() === 'REORDER' && <Tag color="gold" className="u-m-0 u-fs-14 u-lh-16px" style={{ padding: '0 3px', height: 16 }}>翻单</Tag>}
    </>
  ), []);
