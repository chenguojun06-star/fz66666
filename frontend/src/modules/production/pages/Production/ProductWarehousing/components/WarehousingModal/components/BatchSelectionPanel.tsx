...
const BatchSelectionPanel: React.FC<BatchSelectionPanelProps> = ({
    ...
    // 用 useState 替代 Collapse，避免 rc-collapse 对 JSX label 调用
                    ...
                    {batchSelectOpen && (
                        <div style={{ border: '1px solid #d9d9d9', borderTop: 'none', borderRadius: '0 0 6px 6px', padding: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
                            <Space wrap>
                                ...
                                <Button onClick={onSelectClear} disabled={!batchSelectedBundleQrs.length}>清空已选</Button>
                            </Space>
                            ...
                            <div style={{ maxHeight: 280, overflowY: 'auto' }}>
                                <ResizableTable<BatchSelectBundleRow>
                                    ...
                                    columns={[
                                        { title: '菲号', dataIndex: 'qr', width: 220, ellipsis: true },
                                        { title: '扎号', dataIndex: 'bundleNo', width: 80, render: (v: unknown) => (v ? String(v) : '-') },
                                        { title: '颜色', dataIndex: 'color', width: 100, render: (v: unknown) => (String(v || '').trim() ? String(v) : '-') },
                                        { title: '码数', dataIndex: 'size', width: 100, render: (v: unknown) => (String(v || '').trim() ? String(v) : '-') },
                                        { title: '数量', dataIndex: 'quantity', width: 80, render: (v: unknown) => String(Number(v || 0) || 0) },
                                        {
                                            title: '状态', dataIndex: 'statusText', width: 140, ellipsis: true,
                                            render: (v: any, record: BatchSelectBundleRow) => {
                                                ...
                                                const mapped = (key === 'qualified' || key === 'unqualified')
                                                    ? getQualityStatusConfig(key as any) : undefined;
                ...
                <div className="wh-line" style={{ alignItems: 'flex-start' }}>
                    ...
                    <div className="wh-control" style={{ flex: 1 }}>
                        ...
                        >
                            ...
                            <Tag color="geekblue">数量 {summary.totalQty}</Tag>
                        </div>
                        {batchListOpen && (
                            <div style={{
