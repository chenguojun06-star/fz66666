      ...
      <Tabs defaultActiveKey="inventory" style={{ marginTop: 12 }} items={[
        {
          ...
          <StandardModal title={`出库 - ${outboundModal.data?.styleNo || ''}`} open={outboundModal.visible} onOk={handleOutboundConfirm} onCancel={outboundModal.close} okText="确认出库" size="lg">
            {outboundModal.data && (
              <>
                <Card style={{ marginBottom: 12 }}><Row gutter={16}><Col span={8}><div style={{ color: 'var(--color-text-tertiary)', fontSize: 12 }}>订单号</div><div style={{ fontWeight: 600 }}>{outboundModal.data.orderNo || '-'}</div></Col><Col span={8}><div style={{ color: 'var(--color-text-tertiary)', fontSize: 12 }}>款号</div><div style={{ fontWeight: 600 }}>{outboundModal.data.styleNo || '-'}</div></Col><Col span={8}><div style={{ color: 'var(--color-text-tertiary)', fontSize: 12 }}>款名</div><div style={{ fontWeight: 600 }}>{outboundModal.data.styleName || '-'}</div></Col></Row></Card>
                <Card style={{ marginBottom: 12 }}><Row gutter={16}><Col span={12}><div style={{ marginBottom: 8, fontSize: 12, color: 'var(--color-text-tertiary)' }}>出库类型</div><Select style={{ width: '100%' }} value={outboundType} onChange={(v) => setOutboundType(v)} options={[{ label: '销售出库', value: 'sales' }, { label: '调拨出库', value: 'transfer' }, { label: '报废出库', value: 'scrap' }]} /></Col><Col span={12}><div style={{ marginBottom: 8, fontSize: 12, color: 'var(--color-text-tertiary)' }}>{outboundType === 'scrap' ? '报废原因' : '备注（选填）'}</div><Input value={outboundReason} onChange={e => setOutboundReason(e.target.value)} placeholder={outboundType === 'scrap' ? '请填写报废原因' : '选填'} status={outboundType === 'scrap' && !outboundReason.trim() ? 'warning' : undefined} /></Col></Row></Card>
                ...
                <Card title="发货信息（选填）" style={{ marginTop: 12 }}>
                  <Row gutter={16}><Col span={8}><div style={{ marginBottom: 8, fontSize: 12, color: 'var(--color-text-tertiary)' }}>关联生产单号</div><Input value={outboundProductionOrderNo} onChange={e => setOutboundProductionOrderNo(e.target.value)} placeholder="选填" /></Col><Col span={8}><div style={{ marginBottom: 8, fontSize: 12, color: 'var(--color-text-tertiary)' }}>快递单号</div><Input value={outboundTrackingNo} onChange={e => setOutboundTrackingNo(e.target.value)} placeholder="选填" /></Col><Col span={8}><div style={{ marginBottom: 8, fontSize: 12, color: 'var(--color-text-tertiary)' }}>快递公司</div><Input value={outboundExpressCompany} onChange={e => setOutboundExpressCompany(e.target.value)} placeholder="选填" /></Col></Row>
                </Card>
              </>
            )}
          </StandardModal>
          <StandardModal title={`入库记录 - ${inboundHistoryModal.data?.styleNo || ''}`} open={inboundHistoryModal.visible} onCancel={inboundHistoryModal.close} size="lg" footer={null}>
            {inboundHistoryModal.data && (
              <>
                <Card style={{ marginBottom: 12 }}><Row gutter={16}><Col span={8}><div style={{ color: 'var(--color-text-tertiary)', fontSize: 12 }}>款号</div><div style={{ fontWeight: 600 }}>{inboundHistoryModal.data.styleNo || '-'}</div></Col><Col span={8}><div style={{ color: 'var(--color-text-tertiary)', fontSize: 12 }}>总入库量</div><div style={{ fontWeight: 600 }}>{inboundHistoryModal.data.totalInboundQty ?? 0} 件</div></Col><Col span={8}><div style={{ color: 'var(--color-text-tertiary)', fontSize: 12 }}>当前库存</div><div style={{ fontWeight: 600 }}>{inboundHistoryModal.data.availableQty ?? 0} 件</div></Col></Row></Card>
                <ResizableTable size="small" columns={[{ title: '入库日期', dataIndex: 'inboundDate', key: 'inboundDate', width: 120 }, { title: '质检单号', dataIndex: 'qualityInspectionNo', key: 'qualityInspectionNo', width: 140 }, { title: '菲号', dataIndex: 'cuttingBundleNo', key: 'cuttingBundleNo', width: 100 }, { title: '颜色', dataIndex: 'color', key: 'color', width: 80 }, { title: '尺码', dataIndex: 'size', key: 'size', width: 60 }, { title: '数量', dataIndex: 'quantity', key: 'quantity', width: 80, align: 'right' as const }, { title: '操作人', dataIndex: 'operator', key: 'operator', width: 100 }, { title: '库位', dataIndex: 'warehouseLocation', key: 'warehouseLocation', width: 100 }]} dataSource={inboundHistory} rowKey="id" pagination={{ current: inboundPage, pageSize: inboundPageSize, total: inboundHistory.length, onChange: (p, ps) => { setInboundPage(p); setInboundPageSize(ps); } }} />
                <div style={{ marginTop: 12, padding: '8px 12px', background: 'var(--color-bg-container)', borderRadius: 6, fontSize: 13 }}>
