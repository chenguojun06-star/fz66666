...
import { ExportFormat, FORMAT_OPTIONS, EXPORT_TYPES, pageShellStyle } from './taxExportConstants';
...
const TaxExport: React.FC = () => {
  ...
  const handleFormatClick = (opt: typeof FORMAT_OPTIONS[0]) => {
    if (!opt.free && !subscribed) {
      Modal.confirm({
        ...
        content: (
          <div>
            <p className="u-mb-8">金蝶/用友专用格式需要开通<strong>财税对接模块</strong>（¥499/月）。</p>
            <p className="u-fs-14" style={{ color: 'var(--color-text-muted)' }}>通用标准格式永久免费，适合手工导入任意财务软件。</p>
          </div>
        ),
        okText: '前往开通',
        cancelText: '继续用免费版',
  ...
  if (checking) {
    return <div className="u-ta-center" style={{ padding: '80px 0' }}><span>加载中…</span></div>;
  }
          ...
          {EXPORT_TYPES.map(type => (
            <Col xs={24} md={12} key={type.key}>
              <Card style={{ border: `1px solid ${type.color}30`, background: `${type.color}06` }}>
                <div className="u-d-flex u-ai-start u-gap-12">
