    ...
    <>
      ...
      <ResizableModal
        ...
        footer={[
          ...
          <Button key="ok" type="primary" loading={submitting} onClick={() => createForm.validateFields().then(handleCreate).catch((err) => console.error('创建应付款失败:', err))}>保存</Button>,
        ]}
      >
      ...
      >
        <div style={{ padding: '12px 0' }}>
          <p className="u-mb-4">应付金额：<strong>{payRecord?.amount?.toFixed(2)}</strong> 元</p>
          <p className="u-mb-4">已付金额：<strong>{(payRecord?.paidAmount || 0).toFixed(2)}</strong> 元</p>
          <p className="u-mb-16">剩余未付：<strong style={{ color: 'var(--color-warning)' }}>{((payRecord?.amount || 0) - (payRecord?.paidAmount || 0)).toFixed(2)}</strong> 元</p>
          <div>
            <span className="u-mr-8">本次付款金额：</span>
            <InputNumber
              min={0.01}
              ...
              placeholder="输入付款金额"
            />
            <span className="u-ml-8" style={{ color: 'var(--color-text-muted)' }}>元</span>
          </div>
        </div>
      </SmallModal>
    </>
