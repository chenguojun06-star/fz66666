      ...
      <div className="u-p-08px">
          ...
          <Card style={{ marginBottom: 16, background: 'var(--status-success-bg)', border: '1px solid var(--status-success-border)' }}>
            <Descriptions column={2}>
              ...
              <Descriptions.Item label="应付金额">
                <span className="u-fw-600" style={{ color: 'var(--color-error)' }}>{formatMoney(currentPayable.amount)}</span>
              </Descriptions.Item>
            </Descriptions>
          </Card>
        )}
        ...
        <Form form={payForm} layout="vertical" requiredMark="optional">
                  ...
                  >
                    {payeeOptions.map(p => (
                      <Select.Option key={p.id} value={p.id}>
                        <span>{p.name}</span>
                        <span className="u-ml-8 u-fs-14" style={{ color: 'var(--color-text-tertiary)' }}>[{p.label}]{p.phone ? ` ${p.phone}` : ''}</span>
                      </Select.Option>
                    ))}
                  </Select>
                </Form.Item>
          ...
          <Form.Item label="选择支付方式" required>
            <div className="u-d-grid u-gap-12" style={{ gridTemplateColumns: 'repeat(2, 1fr)' }}>
              {PAYMENT_METHOD_OPTIONS.map(opt => (
                ...
                >
                  <div className="u-fs-16 u-mb-4">{methodIconMap[opt.value]}</div>
          ...
          {selectedMethod && selectedMethod !== 'OFFLINE' && (
            <div className="u-br-8 u-p-12 u-mb-16" style={{ background: 'var(--status-success-bg)', border: '1px solid var(--status-success-border)' }}>
              <div className="u-fw-500 u-mb-8">
                收款账户
                <Button
                  type="link"
