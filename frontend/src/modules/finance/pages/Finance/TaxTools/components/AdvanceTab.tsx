...
const AdvanceTab: React.FC = () => {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      const res = await employeeAdvanceApi.list({
        page, size: pageSize, status: filterStatus, employeeName: keyword,
      });
      if (res.code === 200) {
  ...
  const handleReject = (record: EmployeeAdvance) => {
    modal.confirm({
      width: '30vw', title: '驳回借支', content: `确认驳回 ${record.employeeName} 的借支单 ${record.advanceNo}？`,
  ...
  const columns: ColumnsType<EmployeeAdvance> = [
    { title: '借支单号', dataIndex: 'advanceNo', width: 140 },
    ...
    { title: '借支金额', dataIndex: 'amount', width: 100, align: 'right', render: (val: number) => <span style={{ color: 'var(--color-danger)' }}>{formatMoney(val || 0)}</span> },
    { title: '审批状态', dataIndex: 'status', width: 80, render: (val: string) => statusTag(val) },
    { title: '还款状态', dataIndex: 'repaymentStatus', width: 90, render: (val: string) => repaymentTag(val) },
    { title: '剩余金额', dataIndex: 'remainingAmount', width: 100, align: 'right', render: (val: number) => <span style={{ color: 'var(--color-warning)' }}>{formatMoney(val || 0)}</span> },
    { title: '创建时间', dataIndex: 'createTime', width: 110, render: (val: string) => val ? formatDateTime(val) : '-' },
    {
  ...
  return (
    <Space direction="vertical" size={12} style={{ width: '100%' }}>
        ...
        <Row gutter={[12, 12]} align="middle">
          <Col>
            <Select value={filterStatus} onChange={(v) => { setFilterStatus(v); setPage(1); }} allowClear placeholder="审批状态" style={{ width: 120 }}
              options={[{ value: 'pending', label: '待审批' }, { value: 'approved', label: '已审批' }, { value: 'rejected', label: '已驳回' }]} />
          </Col>
          <Col>
      ...
      <ResizableTable storageKey="tax-tools-advance" rowKey="id" columns={columns} dataSource={list} loading={loading} stickyHeader scroll={{ x: 1000 }}
      ...
      >
        <div style={{ padding: '0 8px' }}>
          <Form form={form} layout="vertical" requiredMark="optional">
            <Row gutter={16}>
              <Col span={12}><Form.Item name="employeeName" label="员工姓名" rules={[{ required: true, message: '请填写员工姓名' }]}><Input placeholder="请输入员工姓名" /></Form.Item></Col>
              <Col span={12}><Form.Item name="amount" label="借支金额" rules={[{ required: true, message: '请填写借支金额' }]}><InputNumber min={0.01} precision={2} prefix="¥" placeholder="0.00" style={{ width: '100%' }} /></Form.Item></Col>
            </Row>
            <Form.Item name="reason" label="借支事由" rules={[{ required: true, message: '请填写借支事由' }]}><Input.TextArea rows={3} placeholder="请描述借支原因" /></Form.Item>
            <Form.Item name="orderNo" label="关联订单号"><Input placeholder="选填" /></Form.Item>
          </Form>
        </div>
      </StandardModal>
        ...
        <div style={{ padding: '0 8px' }}>
          ...
          <Form form={repayForm} layout="vertical" requiredMark="optional">
            <Form.Item name="amount" label="还款金额" rules={[{ required: true, message: '请填写还款金额' }]}>
              <InputNumber min={0.01} max={repayRecord?.remainingAmount || repayRecord?.amount || undefined} precision={2} prefix="¥" placeholder="0.00" style={{ width: '100%' }} />
            </Form.Item>
          </Form>
        </div>
      </StandardModal>
