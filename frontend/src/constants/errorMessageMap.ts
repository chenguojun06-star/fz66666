...
const HTTP_STATUS_FALLBACK_MAP: Record<number, string> = {
  400: '请求参数错误，请检查后重试',
  401: '登录已过期，请重新登录',
...
const normalizeErrorCode = (value: string | null | undefined): string => {
  return normalizeText(value).toUpperCase();
};

export const resolveUnifiedErrorMessage = (input: ErrorMappingInput): string => {
  const errorCode = normalizeErrorCode(input.errorCode);
  const serverMessage = normalizeText(input.message);
  const requestId = normalizeText(input.requestId);
  const statusCode = typeof input.statusCode === 'number' ? input.statusCode : 0;
