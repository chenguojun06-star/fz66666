        ...
        return (
          <div
            ...
            style={{
              ...
              background: isActive && card.activeBg
                ? `linear-gradient(135deg, ${card.activeBg} 0%, ${lightenColor(card.activeBg)} 100%)`
                : '#fff',
              ...
              <React.Fragment key={idx}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
                    <span style={{ fontSize: VALUE_FONT_SIZE, fontWeight: 700, color: item.color || '#333' }}>
...
function lightenColor(hex: string): string {
  ...
  // 简单处理：将 #e6f4ff 变为 #f0f7ff 之类
  try {
    const r = parseInt(hex.slice(1, 3), 16);
    const g = parseInt(hex.slice(3, 5), 16);
    const b = parseInt(hex.slice(5, 7), 16);
    const lr = Math.min(255, r + Math.round((255 - r) * 0.4));
    const lg = Math.min(255, g + Math.round((255 - g) * 0.4));
    const lb = Math.min(255, b + Math.round((255 - b) * 0.4));
