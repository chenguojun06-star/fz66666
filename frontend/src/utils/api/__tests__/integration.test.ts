...
const buildJwtLikeToken = (payload: Record<string, unknown>) => {
...
describe('api core integration', () => {
  ...
  it('createApiClient 的请求拦截器会注入请求头', async () => {
    ...
    const validToken = buildJwtLikeToken({ exp: 4102444800, sub: 'user-456' });
    ...
    expect(nextConfig.headers.Authorization).toBe(`Bearer ${validToken}`);
