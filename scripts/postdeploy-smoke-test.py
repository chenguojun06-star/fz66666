...
if not BASE:
    print("=" * 56)
    print("  部署后冒烟测试（Smoke Test）是什么？")
    print("=" * 56)
    print("  冒烟测试 = 部署后用一小批请求摸一遍线上服务，")
    print("  确保'没冒烟起火'（没500/502、历史崩过的接口正常）。")
    print()
    print("  用法（二选一）：")
    print("  1) 本地执行：")
    ...
    print()
    print("  支持的环境变量：")
    print("     SMOKE_BASE_URL    （必填）测试目标，如 https://abc.sh.run.tcloudbaseapp.com")
    print("     SMOKE_USERNAME    （可选）登录账号，默认 admin")
    print("     SMOKE_PASSWORD    （可选）登录密码，默认 admin123")
    print("     SMOKE_TENANT_ID   （可选）租户ID，默认 1")
    print("     SMOKE_TIMEOUT     （可选）单请求超时秒，默认 15")
    print("     SMOKE_MAX_RETRIES（可选）部署后重试次数，默认 6")
    print("     SMOKE_FEISHU_WEBHOOK（可选）失败时飞书通知")
    print()
    print("  💡 快捷：直接复制下面命令改域名即可跑：")
    print("     SMOKE_BASE_URL=https://你的域名 python3 scripts/postdeploy-smoke-test.py")
    print("=" * 56)
    sys.exit(2)
...
def expect_code_200(method, path, name, data=None):
    ...
    try:
        ...
        if biz_code == 200:
            ...
            log("FAIL", name, f"code={biz_code} msg={resp.get('msg','')[:100]}")
            return None
    except Exception as e:
