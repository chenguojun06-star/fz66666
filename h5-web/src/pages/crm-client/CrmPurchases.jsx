...
const CrmPurchases = () => {
  ...
  const loadPurchases = async () => {
    try {
      ...
      const res = await crmClient.getPurchases(params);
      setPurchases(res?.list || []);
      setTotal(res?.total || 0);
      setTotalPages(res?.totalPages || 0);
    } catch (err) {
      console.error('加载采购失败:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={s.page}>
      ...
      {loading ? <div style={s.loading}>加载中...</div> :
       purchases.length === 0 ? <div style={s.empty}>暂无采购信息</div> :
       purchases.map((p) => (
        <div key={p.id} style={s.card} onClick={() => navigate(`/crm-client/purchases/${p.id}`)}>
          ...
          <div style={s.cardRow}><span style={s.cardLabel}>数量</span><span style={s.cardVal}>{p.purchaseQuantity || 0} / 已到 {p.arrivedQuantity || 0}</span></div>
