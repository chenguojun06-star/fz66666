...
| 权限级别 | data_scope | 适用角色 | 数据范围 | 用户要求 |
|---------|-----------|---------|---------|---------|
| **全局权限** | `ALL` | 总部管理员 | 查看所有工厂数据 | `factory_id = NULL` |
| **工厂权限** | `FACTORY_ONLY` | 工厂管理员 | 只看本工厂数据 | `factory_id = '工厂A'` |
| **团队权限** | `TEAM` | 组长 | 只看本团队数据 | - |
| **个人权限** | `OWN` | 普通员工 | 只看个人数据 | - |
...
- 用户切换工厂：需手动 `UPDATE t_user SET factory_id`
- 新增业务表：添加 `factory_id` 字段并在 Service 层应用过滤
