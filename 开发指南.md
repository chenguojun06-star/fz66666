...
**文档版本**: 3.2  
**最后更新**: 2026-04-17  
**编排器数**: 235个（105业务编排器 + 130AI智能体编排器） | **知识库**: 50条 | **AI Skill**: 3个 | **WebSocket事件**: 24种
...
| 模块              | 状态    | 完成度 | 说明                                    |
| ----------------- | ------- | ------ | --------------------------------------- |
| 后端 Orchestrator | ✅ 完成 | 100%   | 105个业务编排器运行稳定，212k行代码       |
| 前端模块化        | ✅ 完成 | 100%   | 10个模块，173k行（TS+CSS），构建通过    |
...
- [后端 Orchestrator 模式](#一后端架构-orchestrator-模式) - 105个业务编排器详解
...
### 1.2 核心 Orchestrator 清单（**105个业务 + 130 AI智能体**）

> 完整的最新编排器清单请参见 **[模块与职责快速查询表.md](模块与职责快速查询表.md)**，本文档仅列模块分布总览。

**按领域模块分布（14个模块，105个业务编排器）：**

| 模块 | 数量 | 说明 |
|------|------|------|
| production（生产） | 31 | 订单管理、扫码、裁剪、质检、进度、产能、预警 |
| finance（财务） | 21 | 工资结算、对账、报销、付款、成本、预算 |
| system（系统） | 15 | 租户、用户、角色、工厂、部门、配置 |
| style（款式） | 9 | 款式信息、BOM、工艺、报价、附件、版本 |
| warehouse（仓库） | 8 | 物料库存、入库、出库、盘点、调拨 |
| intelligence（AI智能体） | 130 | 小云AI对话、知识库、异常检测、智能编排 |
| selection（选款） | 4 | 选款审核、批次、候选 |
| integration（集成） | 3 | 开放API、应用商店、电商订单 |
| dashboard（仪表板） | 3 | 数据看板、趋势分析、异常报告 |
| crm（客户） | 3 | 客户管理、账单聚合 |
| template（模板） | 2 | 生产模板、款式模板 |
| procurement（采购） | 2 | 采购订单、供应商评分 |
| wechat（微信） | 2 | H5授权、小程序授权 |
| datacenter（数据中心） | 1 | 数据同步、质量管理 |
| search（搜索） | 1 | 全局搜索 |

> ⚠️ 注意：此列表为自动统计结果，与代码实际文件保持同步。`intelligence` 模块的130个AI编排器属于小云AI智能体编排，与业务编排器分层隔离。
