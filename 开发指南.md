...
**文档版本**: 3.2  
**最后更新**: 2026-04-17  
**编排器数**: 157个 | **知识库**: 50条 | **AI Skill**: 3个 | **WebSocket事件**: 24种
...
#### 4a. 🚨 带原因输入的确认弹窗 — 必须使用 RejectReasonModal

> **背景**：`modal.confirm()` 内嵌 `Input.TextArea` 会导致按钮被遮挡、不可点击。根本原因：`modal.confirm()` 打开时高度一次性计算，`autoSize` TextArea 动态增高后覆盖 Footer 按钮。**全系统已统一使用 `<RejectReasonModal>` 组件修复。**

```tsx
// ❌ 禁止：modal.confirm + Input.TextArea（按钮会被遮挡）
modal.confirm({
  content: <Input.TextArea autoSize onChange={(e) => { remark = e.target.value; }} />,
  onOk: async () => { await api.call(remark); },
});

// ✅ 正确（组件中）：React 状态 + RejectReasonModal
import RejectReasonModal from '@/components/common/RejectReasonModal';

// 1. 声明 pending 状态
const [pendingDelete, setPendingDelete] = useState<MyItem | null>(null);
const [deleteLoading, setDeleteLoading] = useState(false);

// 2. 触发函数只设置 pending
const handleDelete = (item: MyItem) => setPendingDelete(item);

// 3. 确认函数处理 API
const handleDeleteConfirm = async (reason: string) => {
  setDeleteLoading(true);
  try {
    await api.delete(pendingDelete!.id, reason);
...
<RejectReasonModal
  ...
  fieldLabel="删除原因"
  required={true}          // 默认 true（必填）
  okDanger={true}          // 默认 true（红色确认按钮）
...
**Hook 中的用法**（Hook 不能渲染 JSX，需暴露状态给父组件渲染）：
```tsx
// useMyHook.tsx — 暴露状态
const [pendingAction, setPendingAction] = useState<Item | null>(null);
const [actionLoading, setActionLoading] = useState(false);
const handleAction = useCallback((item: Item) => { /* validate */ setPendingAction(item); }, [...]);
const confirmAction = useCallback(async (remark: string) => { /* API call */ }, [pendingAction, ...]);
const cancelAction = useCallback(() => setPendingAction(null), []);
return { handleAction, pendingAction, actionLoading, confirmAction, cancelAction };

// ParentComponent.tsx — 渲染弹窗
const { handleAction, pendingAction, actionLoading, confirmAction, cancelAction } = useMyHook({...});
<RejectReasonModal
...
**RejectReasonModal Props**：

| Prop | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `open` | `boolean` | — | 是否显示 |
| `title` | `string` | `'确认操作'` | 弹窗标题 |
| `description` | `React.ReactNode` | — | 正文描述内容 |
| `fieldLabel` | `string` | `'退回原因'` | 文本框标签 |
| `required` | `boolean` | `true` | 文本框是否必填 |
| `okDanger` | `boolean` | `true` | 确认按钮是否为红色 |
| `okText` | `string` | `'确认'` | 确认按钮文字 |
| `cancelText` | `string` | `'取消'` | 取消按钮文字 |
| `loading` | `boolean` | `false` | 确认按钮 loading |
| `onOk` | `(reason: string) => void` | — | 确认回调，入参为输入内容 |
| `onCancel` | `() => void` | — | 取消回调 |

**已使用 RejectReasonModal 的页面列表**（不要改回 modal.confirm）：
- `StyleStageControlBar`, `useCuttingTasks`, `Cutting/index`, `RoleList`, `UserList`, `FactoryList`
- `MaterialTable`, `useStyleActions`, `StyleInfoList`, `useProductionActions`, `Production/List/index`
- `TemplateCenter/index`, `BillingTab`, `RegistrationTab`
- `useCloseOrder` (Hook) + `ProgressDetail/index`（父组件渲染）
...
- [ ] **阴影**: 使用标准轻量阴影（避免过重）
- [ ] **❌ 禁止**: `modal.confirm()` 中嵌套 `Input.TextArea` → 改用 `<RejectReasonModal>`
...
> **生产-质检-入库业务流完整性校验（2026-05-06 新增）**

在生产、质检、入库等核心链路中，所有工序流转与节点校验必须通过统一的 `ProductionScanStageSupport`（stageSupport）实现，禁止在各 Executor/Service 内部硬编码节点名。

**关键业务规则：**
- 质检/入库等环节，必须同时校验“车缝”与“尾部”节点已全部完成，任一未完成则拒绝后续操作。
- 所有 stage 校验逻辑必须保证：上游未完成时，下游操作一律禁止。
- 校验失败时，返回统一错误提示，便于前端和用户理解。

**示例场景：**
- 质检扫码时，系统自动校验“车缝”与“尾部”节点是否全部完成，任一未完成则拒绝质检。
- 入库扫码时，同理，必须校验所有前置工序已完成。
- 禁止在 QualityScanExecutor、WarehouseScanExecutor 等类中出现“车缝”“尾部”等硬编码判断，全部通过 stageSupport 统一实现。
...
### 🚨 P0铁律：业务流完整性校验（2026-05-06 新增）

> **所有涉及生产、质检、入库等业务链路的代码变更，必须确保上下游业务流完整性，严禁只改一端或遗漏关键环节。**

**核心规则：**
- 任何涉及工序流转、扫码、质检、入库的校验逻辑，必须统一通过 `ProductionScanStageSupport`（stageSupport）集中校验，禁止在各 Executor/Service 内部硬编码节点名（如“车缝”“尾部”“包装”）。
- 质检/入库等环节，必须同时校验“车缝”与“尾部”节点已全部完成，不能只依赖单一节点。
- 所有 stage 校验逻辑必须保证：
  - 上游未完成时，下游操作一律禁止（如未完成“车缝/尾部”禁止质检/入库）。
  - 校验失败时，返回统一错误提示，便于前端和用户理解。
- 任何变更必须同步补充/修订本手册与系统状态文档，确保所有开发者理解最新业务链路。

**业务场景举例：**
- 质检扫码时，系统会自动校验“车缝”与“尾部”节点是否全部完成，任一未完成则拒绝质检。
- 入库扫码时，同理，必须校验所有前置工序已完成。
- 禁止在 QualityScanExecutor、WarehouseScanExecutor 等类中出现“车缝”“尾部”等硬编码判断，全部通过 stageSupport 统一实现。

**责任归属：**
- 该规则为 P0 级铁律，违反将导致业务流断裂、数据脏污，所有代码评审与上线前必须逐项核查。
