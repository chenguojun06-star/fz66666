...
| 前端模块化 | ⏳ 进行中 | 40% | basic/system/dashboard已完成 |
...
**已完成迁移**：
- ✅ basic 模块（4/4页面）：款号资料、下单管理、资料中心、模板中心
- ✅ system 模块（5/5页面）：用户、角色、审批、日志、个人中心
- ✅ dashboard 模块（1/1页面）：数据概览

**待迁移**（约15个页面）：
- ⏳ production 模块：生产订单、裁剪单、质检入库、物料采购、进度跟踪
- ⏳ finance 模块：物料对账、成品结算、发货对账、员工工序、审批付款

**迁移优先级**：
1. 🔥 高优先级：生产订单、裁剪单（核心业务）
2. 📊 中优先级：财务对账（使用频繁）
3. 📝 低优先级：报表统计（使用较少）
