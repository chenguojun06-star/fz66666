...
系统存在敏感财务数据（单价、工资、金额等），需要基于角色控制可见性：

**📊 敏感数据访问权限矩阵**：

| 角色 | role_code | 能看订单？ | 能看单价？ | 能看金额？ | 能看别人工资？ | 扫码功能 |
|-----|----------|---------|----------|----------|------------|---------|
| **管理员** | admin | ✅ 所有订单 | ✅ 能看 | ✅ 能看 | ✅ 能看所有人 | ✅ 正常 |
| **采购员** | purchaser | ✅ 本部门订单 | ✅ 能看 | ✅ 能看 | ✅ 能看本部门 | ✅ 正常 |
| **主管** | supervisor | ✅ 本部门订单 | ✅ 能看 | ✅ 能看 | ✅ 能看本部门 | ✅ 正常 |
| **裁剪员** | cutter | ✅ 本部门订单 | ❌ 显示*** | ❌ 显示*** | ❌ 只看自己 | ✅ 正常 |
| **车缝员** | sewing | ✅ 本部门订单 | ❌ 显示*** | ❌ 显示*** | ❌ 只看自己 | ✅ 正常 |
| **质检员** | quality | ✅ 本部门订单 | ❌ 显示*** | ❌ 显示*** | ❌ 只看自己 | ✅ 正常 |

**🔄 权限配置流程**：

```
第1步: 添加权限数据        第2步: 分配角色权限        第3步: 前端判断显示
┌──────────────────┐      ┌──────────────────┐      ┌──────────────────┐
│ INSERT INTO      │      │ INSERT INTO      │      │ if (canViewPrice)│
│ t_permission     │  →   │ t_role_permission│  →   │   显示真实单价   │
│ (PRICE_VIEW)     │      │ (admin/purchaser)│      │ else 显示 ***    │
└──────────────────┘      └──────────────────┘      └──────────────────┘
```
...
WHERE r.role_code IN ('admin', 'supervisor', 'purchaser')
AND p.permission_code = 'FINANCE_PRICE_VIEW';

-- ❌ 普通员工（裁剪员、车缝员、质检员）不分配此权限
```

**2️⃣ 前端权限判断（React/TypeScript）**：
...
// 判断是否有查看单价权限
const canViewPrice = userPermissions.includes('FINANCE_PRICE_VIEW');
  ...
  ...(canViewPrice ? [
    {
      ...
      dataIndex: 'unitPrice',
      key: 'unitPrice',
      render: (value: number) => toMoneyText(value)
    },
    {
      title: '金额(元)',
      dataIndex: 'amount',
...
│    b. 查询工序配置（含单价）   ✅                            │
│    c. 验证员工扫码权限         ✅                            │
│    d. 计算工序金额：quantity × unitPrice                     │
│    ↓                                                         │
│ 4. 💾 保存扫码记录到数据库：                                 │
│    INSERT INTO t_scan_record (                               │
│      order_no,        -- PO20260125003                      │
│      process_name,    -- 做领                                │
│      operator_id,     -- 123（员工ID）                       │
│      quantity,        -- 50                                 │
│      unit_price,      -- 1.5（后端自动填充）✅               │
│      scan_cost        -- 75（自动计算）✅                    │
│    )                                                         │
│    ↓                                                         │
│ 5. 📱 小程序显示（前端控制）：                               │
...
| 页面 | 路径 | 敏感字段 | 需要权限控制 |
|-----|------|---------|------------|
| 员工工资汇总 | `/pages/Finance/PayrollOperatorSummary.tsx` | unitPrice, amount | ✅ 是 |
| 订单管理 | `/modules/basic/pages/OrderManagement/index.tsx` | unitPrice, totalAmount | ✅ 是 |
| 裁剪单 | `/modules/production/pages/Production/Cutting/index.tsx` | unitPrice | ✅ 是 |
| 成品结算 | `/modules/finance/pages/FinishedSettlement/index.tsx` | unitPrice, totalAmount | ✅ 是 |
| 款式信息 | `/modules/basic/pages/StyleInfo/index.tsx` | price | ✅ 是 |
| 物料对账 | `/components/Finance/MaterialReconModalContent.tsx` | unitPrice | ✅ 是 |
