...
Page({
  ...
  onResultItemTap(e) {
    ...
    let targetTab = 'orders_all'; // 默认显示全部订单

    if (status === 'production' || status === 'cutting' || status === 'sewing') {
      targetTab = 'orders_production'; // 生产中
    } else if (status === 'draft' || status === 'pending') {
      targetTab = 'orders_all'; // 全部订单
    }
