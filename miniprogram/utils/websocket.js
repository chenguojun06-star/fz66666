  ...
  try {
    ...
    if (!result.ok) {
      ...
      if (tokenAfterLogin && !isTokenExpired()) {
        if (DEBUG) console.log('[WebSocket] refreshToken 刷新失败但用户已重新登录，使用新 token 继续');
...
/**
 * 触发事件（带防抖）
 * @param {string} eventType - EventBus 事件类型
 * @param {object} data - 事件数据
 */
function emitWithDebounce(eventType, data) {
  if (debounceTimers[eventType]) {
    clearTimeout(debounceTimers[eventType]);
  }
  debounceTimers[eventType] = setTimeout(function () {
  ...
  try {
    ...
    if (mappedEvents) {
      for (var i = 0; i < mappedEvents.length; i++) {
        emitWithDebounce(mappedEvents[i], emitData);
      }
    } else {
      emitWithDebounce(Events.DATA_CHANGED, emitData);
    }
