...
> 最后更新：2026-08-25（D-126 供应商准入闭环补全 + 财务四链路审查完成）

## 已完成

### 2026-08-25 D-126 供应商准入闭环补全 ✅（tsc 0 errors，待部署验证）

- [x] AdmissionAuditModal 审核弹窗（通过/试用/拒绝/暂停，拒绝暂停必填原因）；RowActions 入口仅 isAdmin 可见
- [x] 统计口径修正：空状态计已准入；V202608250001 回填老数据 approved
- [x] cloudflared 安装完成，外网隧道恢复
- [ ] 部署后验证：待审核行出现"准入审核"，审核后统计卡与表格一致
