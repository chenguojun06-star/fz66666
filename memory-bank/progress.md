...
### 2026-08-25 D-132 外发应付砍双轨 ✅（tsc/mvn/flyway 全过，待部署验证）

- [x] 留成品结算轨（活UI+锁定单价+终审推送），砍出货对账单的应付推送（三处：两包 pushReceivableBill 外发分支+审批外发分支）
- [x] 对账单保留扣款明细/备注载体+销售应收两个职责；删死组件 ShipmentReconContent
- [x] V202608250003 作废遗留 PENDING 的重复应付账单（已确认/已付的不动）
- [ ] 部署后验证：外发出库不再产生 EXTERNAL_FACTORY 应付账单；成品结算终审推送照常
