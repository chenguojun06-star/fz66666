...
- [x] 网站重启后恢复（旧版本，容器自愈策略生效）
- [x] autodeploy.sh 修复本地完成，`bash -n` 语法通过
- [ ] 服务器加 swap + 暂停 cron（控制台网页终端执行）
- [ ] 推送修复 → 手动串行部署 `d6fd94b11` → 验证版本水印后恢复 cron
- [ ] 观察后续 2~3 次自动部署内存曲线（考虑给 Maven `-T1`、Node `--max-old-space-size` 进一步压峰值）
