...
|------|-----|------|
| 日期 | 2026-07-10 | — |
| 当日主题 | 小程序/UI/性能/扫码全量优化 | — |
| 修改文件数 | 74+ 个（小程序/UI 样式为主） | — |
| Bug修复 | 15+ 项 | ✅ 已修复 |
| UI/样式调整 | 10+ 项（去 emoji、镂空按钮、字体统一、搜索框统一等） | ✅ 已验证 |
| 性能优化 | 12 处（N+1 + 全表扫描） | ✅ 已优化 |
| 前端类型检查 | `npx tsc --noEmit` 0 errors | ✅ 通过 |
| 前端生产构建 | `npm run build` exit 0 | ✅ 通过 |

**今日操作日志（按主题）**：
- iOS 日期兼容与样衣扫码流程优化
- 后端 5 处 N+1 + 7 处 RiskDetector 全表扫描性能修复
- 工序进度条数量信息显示
- 小程序全局 UI 专业化（74 文件：去 emoji、SVG 化、镂空按钮、统一字体/搜索框/颜色）
- 订单详情页图片轮播、样衣详情附件预览
- 设计预览页面与问题评审修复
- 按钮/输入框高度统一（11 处硬编码 + 22 处实心按钮改镂空）
- 采购/样衣/裁剪/生产管理交互 bug 修复
- 样衣开发与采购节点数据联动（新增 quick-edit / stock-check 接口）
- 已关闭订单采购记录过滤
- WebSocket 日志级别与 /error 500 排查
- 采购表格勾选后序号列消失修复（global.css）
