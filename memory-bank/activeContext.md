...
> 最后更新：2026-08-25（D-132 外发应付砍双轨）

---

## 最近变更（Latest Changes）

### 2026-08-25 ★★★ D-132 外发应付砍双轨 ✅全绿待部署

- [x] 用户拍板"留一个砍一个"：留成品结算轨（活UI+锁定单价+终审推送），砍出货对账单的应付推送
- [x] 三处停推外发应付（production/finance 两包 pushReceivableBill 外发分支 + 审批外发分支）；对账单保留扣款明细/备注载体+销售应收职责；删死组件 ShipmentReconContent；V202608250003 作废遗留 PENDING 重复账单（已确认/已付的不动）
- [ ] 待线上验证：外发出库不再产生 EXTERNAL_FACTORY 应付账单；成品结算终审推送照常
