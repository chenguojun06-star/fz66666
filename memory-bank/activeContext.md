...
> 最后更新：2026-08-25（D-126 供应商准入闭环补全 + 财务四链路审查）

---

## 最近变更（Latest Changes）

### 2026-08-25 ★★ D-126 供应商准入闭环补全 ✅tsc 全过待部署

- [x] 准入功能此前是半成品：后端审核接口前端零调用（状态卡死）、统计卡把空值算待审核（数字对不上）、老数据无回填
- [x] AdmissionAuditModal（四结果+意见，拒绝/暂停必填原因）+ RowActions 入口（isAdmin）+ 统计口径修正 + V202608250001 回填 approved
- [ ] 待线上验证：待审核行"更多"里出现准入审核，审核后卡片与表格一致

### 2026-08-25 ★★★ 财务四链路审查完成（未改码，报告待用户拍板）

- [x] 内部工资：双支付通道 bizId 错位（付款后状态永不更新）、页面聚合与结算单两套口径、记录打款/扣款按钮必失败（死功能）
- [x] 外发结算：出货对账单与成品结算双轨应付（重复付款风险）、付工厂按售价而非加工费、外发应付交易对手写成客户、次品扣款恒为零
- [x] 物料采购：采购总额 4 种口径互相覆盖（新单落库即 0 元）、入库→对账同步失败静默吞、对账列表展示层二次覆盖存储金额
- [x] 成品次品：次品自动扣款传零成本永不生效、返修成功不撤销扣款（死代码）、出库类型前后端词汇表不一致、自由出库账外应收孤岛
- [ ] 待用户确定优化优先级；最高优先建议：统一"单一事实源"（外发=订单锁定加工价×合格回料数−有效扣款；工资=t_payroll_settlement 唯一载体）
