...
### AP-AI-04: 外部 AI/Embedding 接口只对 404/401 熔断，429 限流不熔断
**识别信号**：日志里反复出现 `429 Too Many Requests` + `降级为伪向量`，且每次耗时几秒（远超阈值）；
`docker logs | grep -c "429"` 一天上百次。典型代码：`if (msg.contains("404") || msg.contains("401")) broken = true;`
—— 只有 429 不在判定里。
**错误做法**：只对"不会自愈"的配置类错误（404/401/403）熔断，**限流类错误不熔断**。
于是限流期间每次请求都照发 → 被拒 → 白等超时（实测 2.9~4.4 秒）→ 最后还是走兜底。
**正确做法**：把失败分三类，分别处理：
- **PERMANENT**（404/401/403/402）：不会自愈 → 本次运行内永久熔断；
- **RATE_LIMITED**（429 / Too Many Requests / rate limit / quota / throttl）：**会自愈** →
  连续 N 次后**冷却熔断** + 冷却结束**半开放一次探测** + 成功则复位；
- **TRANSIENT**（超时/连接重置）：重试有意义 → 不熔断。
冷却时长要设**下限**（如 ≥1000ms），否则误配成 0 会让熔断形同虚设。
**历史教训**：2026-09-24 线上实测，智谱 embedding-3 打满配额后 24h 内 122 次 429、62 次降级，
每次白等 3~4 秒 —— 配额照耗、时间照花、结果一模一样还是伪向量（见 D-539）。
**触发P0铁律**：凡调外部 API 且有本地兜底的，失败必须分类处理；限流必须熔断且必须能自愈。
