...
```
3. 🚨 **`SET @s = IF(...)` 块内的 SQL 字符串禁止使用 `COMMENT ''text''`**  
...
4. 脚本本地执行通过后再 push，不再需要手动在控制台执行（Flyway 会自动迁移）
...
根本原因深挖：
  ...
  但实际列从未添加到云端 DB —— 这是 Silent failure，flyway_schema_history 显示成功但列缺失。

  同时：V20260424001 ALTER TABLE 段从未包含 unqualified_quantity（仅在 CREATE TABLE IF NOT EXISTS 中），
  导致 t_product_warehousing 已存在时此列也从未被补充。

Flyway 修复（V202607192304，无 COMMENT，全部幂等）：
  + t_cutting_task.factory_type          VARCHAR(20) DEFAULT NULL
  + t_product_warehousing.repair_status  VARCHAR(30) DEFAULT NULL
  + t_product_warehousing.repair_operator_name   VARCHAR(50) DEFAULT NULL
  + t_product_warehousing.repair_completed_time  DATETIME DEFAULT NULL
  + t_product_warehousing.unqualified_quantity   INT NOT NULL DEFAULT 0
  + 回填 UPDATE repair_status='pending_repair' WHERE unqualified_quantity > 0 AND repair_status IS NULL
...
代码防御 select（双保险，即使云端 DB 列补充需重启才生效期间也不报错）：
  ✅ ProductionOrderOrchestrator [次品数量] 查询：
     ...
     — 仅取 2 列，彻底绕开 repair_status 系列缺列
  ✅ OrderCuttingFillService.queryCuttingTasks()：
     .select(CuttingTask::getId, CuttingTask::getProductionOrderId, ...24 列...)
     — 排除 factory_type，列举所有其他稳定业务字段

涉及文件（第二阶段）：
  📄 backend/src/main/resources/db/migration/V202607192304__fix_cloud_missing_columns_no_comment.sql（新建）
