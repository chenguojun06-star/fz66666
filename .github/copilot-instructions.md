...
> **P0 铁律：所有测试代码（单元测试、集成测试、测试脚本）仅在本地开发机保留，严禁提交到 Git 仓库。**
>
> - ✅ 本地保留：`backend/src/test/`（107个测试文件）、`frontend/src/**/*.test.ts`、`miniprogram/test/`
> - ❌ 禁止提交：所有 `*.test.*`、`*.spec.*`、`/test/` 目录文件一律通过 `.gitignore` 隔离
> - 验证：`git ls-files backend/src/test/ | wc -l` 结果必须为 0（仓库中无测试代码）
> - 云端部署物必须零测试代码（节省容器镜像大小 \~50MB，加速启动，避免云端干扰）

### 业务流程完整性校验铁律（P0，扫码/工序/质检/入库/多端）

> **P0 铁律：凡涉及扫码、工序流转、质检、入库、PC端与小程序端等业务链路的代码修改，必须全链路校验上下游数据一致性与工序依赖。**
>
> - 任何工序/节点/状态流转相关改动，必须保证所有相关链路（如扫码→工序→质检→入库）数据同步、依赖关系正确。
> - 禁止只改一端（如仅改扫码/仅改PC端/仅改小程序端/仅改质检/仅改入库），必须同步验证所有上下游环节。
> - PC端与小程序端的工序/节点/状态流转规则必须同步校验，防止数据不一致或业务断链。
...
| 🔴 P0 | **权限码虚构**            | t\_permission 表不存在的权限码                                            | **全员 403**                                  | 见「权限控制模式」                   |
...
> **数据库一致性检查（强制，P0规律第6条）**：
>
> - ✅ **新增 Entity 字段**：必须同时在 `db/migration/V*.sql` 新增或修改表列（使用 INFORMATION\_SCHEMA 幂等写法）
> - ✅ **新增 Flyway 脚本**：检查是否对应新的 Entity 字段（扫码后向下游检查）
> - ✅ **修改现有表结构**：优先使用幂等 INFORMATION\_SCHEMA 判断 + SET @s IF()，不要直接 ALTER TABLE
...
> **云端 schema 漂移防线补充（2026-03-22 新增）**：
>
> - ✅ `CoreSchemaPreflightChecker` 必须把高频业务链路涉及的关键列纳入白名单，至少覆盖 `t_cutting_task.factory_type`、`t_product_warehousing.repair_status / repair_operator_name / repair_completed_time / unqualified_quantity`
> - ✅ `DbColumnRepairRunner` 必须同步覆盖同一批高频缺列，形成“启动预检 + 启动自愈”双保险，不能只靠页面报错后再排查
> - ✅ 对首页、铃铛、扫码页会高并发命中的表，新增 DB 列后要优先补到预检/自愈组件，而不是只写 Flyway
> - ❌ 禁止把高频业务表的新列仅留在 Entity/Flyway 中，未进入预检或启动修复名单就上线
...
□ 是否涉及扫码、工序、质检、入库、PC端/小程序端等多链路？→ 必须全链路校验上下游数据一致性与工序依赖
...
□ 涉及扫码/工序/质检/入库/多端业务链路，是否已全链路校验上下游数据一致性与工序依赖？（只改一端禁止推送）
...
1. 【P0铁律】扫码/工序/质检/入库/PC端/小程序端等业务链路，禁止只改一端，必须全链路校验上下游数据一致性与工序依赖，否则极易导致业务断链、数据不一致、生产/质检/入库异常。
2. **【禁止】修改内网配置**：`vite.config.ts` 中 `hmr.host='192.168.2.215'` 和 `dev-public.sh` 中 `--host 0.0.0.0` 是固定配置，修改会导致动态模块导入失败和 API 代理异常
3. **403 错误**：未使用 `./dev-public.sh` 启动，缺少 `.run/backend.env` 环境变量
4. **数据库连接失败**：检查端口是否为 3308（非标准 3306），容器名 `fashion-mysql-simple`
5. **使用废弃 API**：检查 `@Deprecated` 标记，所有新代码必须使用 `POST /list` 和 `stage-action` 模式
6. **弹窗尺寸不统一**：必须使用三级尺寸（60vw/40vw/30vw），禁止自定义
7. **Service 互调**：必须通过 Orchestrator，否则无法进行事务管理
8. **扫码重复提交**：理解防重复算法，不要随意修改时间间隔
9. **跨端验证不一致**：修改 validationRules 时必须同步 PC 端和小程序
...
14. **云端 Flyway 自动运行**：`FLYWAY_ENABLED=true`（cloudbaserc.json 已配置），所有 `V*.sql` Flyway 脚本**每次容器重启时自动执行**。push 到 main → CI/CD 部署 → Flyway 自动完成数据库迁移，**无需手动在控制台执行 SQL**。本地开发环境默认 `FLYWAY_ENABLED=false`（application.yml 默认值），由 `DbColumnRepairRunner` 启动自愈保障本地字段完整。⚠️ *铁则：禁止修改已执行过的 V*.sql 文件内容\*（checksum 校验失败 → Spring Boot 不启动 → 全部 API 500），发现问题只能新建补偿脚本。
15. **git push = 云端自动重新部署**：`.github/workflows/ci.yml` 的 `deploy` job 通过腾讯云 `cloudbase-action` 触发部署，push 到 main 后 3\~5 分钟自动生效。**需要** GitHub Actions Secrets（`CLOUDBASE_SECRET_ID` / `CLOUDBASE_SECRET_KEY` / `CLOUDBASE_ENV_ID`，已在仓库 Settings 中配置），**无需**手动上传 JAR。
16. **Java 类型安全**：使用 `UserContext.tenantId()` 等工具方法前必须确认返回类型（返回 `Long`，不是 `String`）。编写新 Orchestrator 时，查阅同模块已有编排器的实际调用方式，不要凭记忆猜测类型。
17. **【2026-03-11/12】ViewMigrator 视图 Collation NONE 错误（已通过 Flyway 彻底修复）**：`v_production_order_flow_stage_snapshot` 等视图中 `CONVERT(... USING utf8mb4)` 不加 `COLLATE` 会产生 NONE 可强制性（coercibility 6），`MAX()` 对多行聚合时 MySQL 8.0 抛出 `Illegal mix of collations (utf8mb4_bin,NONE) for operation 'max'`。
...
- ✅ **统一格式**：`V{YYYYMMDDHHMM}__description.sql`（12位，精确到分钟，**必须使用创建当天的真实日期**）
  - 示例：`V202604201430__add_style_tags.sql`（2026-04-20 14:30 实际创建）
  - 天然唯一、按时间排序、不需要序号后缀
- ❌ **旧格式（禁止新增）**：`V{YYYYMMDDNNN}`（日期+3位序号，如 `V20260510001`）
  - 旧格式已造成日期虚高（实际3月却有5月版本号）+ 序号无限堆积，禁止延续
- ❌ **禁止使用未来日期**：AI 生成脚本时必须使用当前真实日期，严禁编造未来日期（如写 `202612` 实际只是4月）
- **关于现存虚高日期脚本**（`V202608~V202612`）：均为历史 AI 失误，已推送执行，无法重命名（Flyway checksum保护）。**接受现状**，后续用真实日期新建脚本即可。
- **`out-of-order: true` 兜底**：Flyway 已配置允许乱序执行，`V202604201430`（4月新建）完全可以在 `V202612020000`（旧有12月脚本）之后被正常执行，无需担心"顺序倒了"。
- **新旧混用说明**：12位时间戳（`202604201430`）与旧格式排序兼容，无需迁移旧脚本

**强制规范**：

1. `CREATE TABLE` 必须用 `CREATE TABLE IF NOT EXISTS`
2. `ALTER TABLE ADD COLUMN` **禁止直接写** → 必须用 INFORMATION\_SCHEMA 条件判断（MySQL 8.0 不支持 `IF NOT EXISTS`）：
...
- INVENTORY\_SYSTEM\_GUIDE.md - 进销存操作
...
- 进销存系统：INVENTORY\_SYSTEM\_GUIDE.md
...
| 2  | 🟡 数据库补全 | 登录 500（avatar\_url 列缺失）          | `backend/.../db/migration/V20260225__add_user_avatar_url.sql`（已有）                   | 确认已覆盖         | ✅ 无重复         |
| 3  | 🟡 数据库补全 | 登录日志截断（error\_message VARCHAR太短） | `backend/.../db/migration/V20260226b__fix_login_log_error_message.sql`              | **新增**        | ✅ 补全遗漏        |
...
#### 变更 #H ｜ 🔵 权限精细化 — 月度经营汇总权限码新增（INTELLIGENCE\_MONTHLY\_VIEW）
...
### 2026-03-22 变更批次（补充修复：工厂数据隔离缺失）

#### 变更 #⚠️ 🔴 BUG修复 — 今日预警和小云面板未按工厂隔离数据

```
触发问题：
  工厂账号（如"演示工人小李"）登录后，今日预警弹窗和小云 AI 助手面板显示了
  所有工厂的订单，而非仅本工厂订单。例如：
    - 登录账号: "演示工人小李"，所属工厂: "外发工厂"
    - 今日预警中显示: PO20260315001（最美服装工厂, 36%, 0天）— 不属于"外发工厂"
    - 小云助手显示: PO20260315001/PO20260315002/PO20260316001（都来自最美服装工厂）— 错误

根本原因（三层缺失隔离）：
  1️⃣ DailyBriefOrchestrator.getBrief() - highRisk 查询层：
     有 tenantId 隔离，缺少 factoryId 隔离
     → 导致获取了全租户、全工厂的高风险订单
  
  2️⃣ DashboardQueryServiceImpl.listOverdueOrders() 查询层：
     既无 tenantId 隔离，也无 factoryId 隔离
     → 导致获取了全系统的逾期订单列表
  
  3️⃣ 数据源：
     ProductionOrder 实体有 factoryId 字段（line 83）
     UserContext.factoryId() 静态方法可获取当前登录用户的工厂 ID
     但这两处查询都未使用

修复方案：
  ✅ 修改 DailyBriefOrchestrator.java (line 115-135 附近)：
     在 highRisk LambdaQueryWrapper 中新增条件：
     .eq(StringUtils.hasText(briefFactoryId), ProductionOrder::getFactoryId, briefFactoryId)
     这样工厂账号只能看自己工厂的高风险订单
  
  ✅ 修改 DashboardQueryServiceImpl.java (line 338-360 附近)：
     在 listOverdueOrders() 中新增两行隔离：
...
修复后表现：
  ✅ 工厂账号登录后：
     - 今日预警只显示本工厂的逾期订单
     - 小云助手的高风险订单、待办事项仅来自本工厂
     - 对应数据完全不可见其他工厂信息
  
  ✅ 超管/租户主账号（无 factoryId）：
     - 条件判断为 false，查询全量订单
     - 行为不变，保持既有权限层级
...
对系统的帮助：
  🎯 工厂工人工作界面完全隔离，不会看到其他工厂的订单
  🎯 符合多租户多工厂的安全隔离规范
  🎯 消除跨工厂数据泄露风险
  🎯 提升工人的工作聚焦力，减少不必要的任务滚动

废弃代码清查：✅ 纯新增隔离条件，无废弃代码
...
#### 变更 #🩹 🔴 BUG修复 — factory\_type / repair\_status 缺列导致日志风暴与热点接口反复 500
...
#### 变更 #🩹②（第二阶段，commit 65e3d4e7） — 根源修复：V202607192304补列 + 双文件防御select

```

补充说明（在 #🩹 基础上的根源修复，二者配合才能彻底消除 WARN）：

根本原因深挖：
V202607192303 和 V202607192200 中所有 SET @s = IF(... 'ALTER TABLE ... COMMENT ''xxx'''...)
Flyway SQL 解析器将第一个 '' 识别为字符串结束符，完整 ALTER TABLE 语句被截断。
PREPARE stmt FROM @s 执行截断后的 SELECT 1，不报错，Flyway checksum 记录成功，
但实际列从未添加到云端 DB —— 这是 Silent failure，flyway\_schema\_history 显示成功但列缺失。

同时：V20260424001 ALTER TABLE 段从未包含 unqualified\_quantity（仅在 CREATE TABLE IF NOT EXISTS 中），
导致 t\_product\_warehousing 已存在时此列也从未被补充。

Flyway 修复（V202607192304，无 COMMENT，全部幂等）：

- t\_cutting\_task.factory\_type          VARCHAR(20) DEFAULT NULL
- t\_product\_warehousing.repair\_status  VARCHAR(30) DEFAULT NULL
- t\_product\_warehousing.repair\_operator\_name   VARCHAR(50) DEFAULT NULL
- t\_product\_warehousing.repair\_completed\_time  DATETIME DEFAULT NULL
- t\_product\_warehousing.unqualified\_quantity   INT NOT NULL DEFAULT 0
- 回填 UPDATE repair\_status='pending\_repair' WHERE unqualified\_quantity > 0 AND repair\_status IS NULL

代码防御 select（双保险，即使云端 DB 列补充需重启才生效期间也不报错）：
✅ ProductionOrderOrchestrator \[次品数量] 查询：
.select(ProductWarehousing::getOrderId, ProductWarehousing::getUnqualifiedQuantity)
— 仅取 2 列，彻底绕开 repair\_status 系列缺列
✅ OrderCuttingFillService.queryCuttingTasks()：
.select(CuttingTask::getId, CuttingTask::getProductionOrderId, ...24 列...)
— 排除 factory\_type，列举所有其他稳定业务字段

涉及文件（第二阶段）：
📄 backend/src/main/resources/db/migration/V202607192304\_\_fix\_cloud\_missing\_columns\_no\_comment.sql（新建）
📄 backend/src/main/java/com/fashion/supplychain/production/orchestration/ProductionOrderOrchestrator.java
📄 backend/src/main/java/com/fashion/supplychain/production/service/OrderCuttingFillService.java
...
4. INSERT SQL 包含列: progress\_workflow\_json = '{"stages":\[...]}'
5. 云端 DB 无此列 → Unknown column 'progress\_workflow\_json' 异常
...
为什么列不存在于云端：
ProductionOrder.java 映射了 5 个真实 DB 列（@TableField("progress\_workflow\_xxx")），
这些列是本地手动 ALTER TABLE 添加的，从未有任何 Flyway 迁移脚本覆盖。
grep -r "progress\_workflow" db/migration/ → 零结果（问题发现方式）

修复：
📄 backend/src/main/resources/db/migration/
V20260418001\_\_add\_production\_order\_workflow\_fields.sql  (新增)
使用 INFORMATION\_SCHEMA SET @s IF 幂等模式新增全部 5 列：
progress\_workflow\_json        LONGTEXT DEFAULT NULL
progress\_workflow\_locked      INT NOT NULL DEFAULT 0
progress\_workflow\_locked\_at   DATETIME DEFAULT NULL
progress\_workflow\_locked\_by   VARCHAR(36) DEFAULT NULL
progress\_workflow\_locked\_by\_name VARCHAR(50) DEFAULT NULL
...
内部去重，不影响当前功能。正确做法：添加 customer\_id 列并修改注解，待后续迁移。
...
分工说明：
IntelligenceObservabilityOrchestrator 写本地 DB 度量（t\_intelligence\_metrics）
