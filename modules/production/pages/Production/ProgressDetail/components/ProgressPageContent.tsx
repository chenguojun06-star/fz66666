...
const ProgressPageContent: React.FC<ProgressPageContentProps> = ({
  ...
  const [patrolAlertDismissed, setPatrolAlertDismissed] = useState(false);
  const prevRiskKeys = useRef<string>('');
  ...
  useEffect(() => {
    const keys = Object.keys(patrolRiskMap).sort().join(',');
    if (keys !== prevRiskKeys.current) {
      prevRiskKeys.current = keys;
      setPatrolAlertDismissed(false);
    }
  }, [patrolRiskMap]);
  ...
  const patrolAlertBanner = useMemo(() => {
    if (patrolAlertDismissed) return null;
    const orderNos = sortedOrders.map(o => o.orderNo).filter(Boolean) as string[];
    ...
    const msg = highCount > 0
      ? `AI鍙戠幇 ${riskyOrders.length} 涓闄╄鍗曪紙${highCount} 涓珮椋庨櫓锛塦
      : `AI鍏虫敞鍒?${riskyOrders.length} 涓鍗昤;
    const autoInfo = patrolSummary && patrolSummary.autoExecutedToday > 0
      ? `锛屼粖鏃ュ凡鑷姩澶勭悊 ${patrolSummary.autoExecutedToday} 椤筦
      : '';
    return (
      <Alert
        type="warning"
        ...
        closable
        onClose={() => setPatrolAlertDismissed(true)}
      />
    );
  }, [sortedOrders, patrolRiskMap, patrolSummary, patrolAlertDismissed]);
