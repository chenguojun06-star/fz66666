...
### feat(wechat): 新增 WechatWorkNotifyService，逾期/停滞/AI异常实时推送到企业微信工作群

#### 新增文件

| 文件 | 说明 |
|------|------|
| `wechat/service/WechatWorkNotifyService.java` | 企业微信群机器人 Webhook HTTP 推送服务，支持 Markdown 消息 |

#### 修改文件

| 文件 | 变更 |
|------|------|
| `production/orchestration/SysNoticeOrchestrator.java` | `sendAuto()`（逾期/停滞）和 `sendAnomalyToManagers()`（AI异常检测）中新增企业微信 Webhook 旁路推送 |
| `backend/src/main/resources/application.yml` | 新增 `wechat.work.enabled` / `wechat.work.webhook-url` 配置项 |

#### 功能说明
- **零侵入**：企业微信推送通过 try-catch 包裹，失败只记录 WARN 日志，绝不影响主业务流程
- **配置即用**：添加环境变量 `WECHAT_WORK_NOTIFY_ENABLED=true` + `WECHAT_WORK_WEBHOOK_URL=<url>` 即可开启
- **三触发点**：
  - `sendAuto()` — AI 调度器检测到逾期/停滞时，Webhook 同步推送标题+详情到工作群
  - `sendAnomalyToManagers()` — AI 异常检测触发时推送，与站内通知并行
  - `sendOrderAlert()` — 可复用的单订单预警方法，支持 deadline/stagnant/anomaly/danger_alert 四种类型
- **Markdown 格式**：支持企业微信原生语法（加粗/颜色），关键信息直观呈现
...
wechat:
  work:
    ...
    webhook-url: ${WECHAT_WORK_WEBHOOK_URL:}
```
```bash
# .run/backend.env（本地开发）或云端环境变量
WECHAT_WORK_NOTIFY_ENABLED=true
...
#### 对系统的帮助
- 生产异常从"只有跟单员登录才能看到"提升为"实时群通知"，响应速度由小时级→秒级
- 无需开通企业微信企业内部应用，群机器人免费且零配置

---

## ✨ MCP SSE Transport + Token 换取接口 — 小云对外开放标准接入通道

### feat(intelligence): 实现 MCP 2024-11-05 SSE Transport，支持 Cursor/Claude Desktop 等外部客户端接入小云

#### 新增内容

| 文件 | 端点 | 说明 |
|------|------|------|
| `McpAuthController.java` | `POST /api/intelligence/mcp/token` | **公开端点**（无需 JWT）；用 AppKey + HMAC-SHA256 签名换取 90 天 MCP Token |
| `McpSseController.java` | `GET /api/intelligence/mcp/sse?token=<jwt>` | SSE 连接，发送 `event: endpoint` 把消息地址告知客户端 |
| `McpSseController.java` | `POST /api/intelligence/mcp/messages?sessionId=<sid>` | 接收 JSON-RPC 2.0，路由到 McpProtocolService，响应写回 SSE 流 |
| `McpSseSessionService.java` | 内部服务 | 管理 `ConcurrentHashMap<sessionId, SseEmitter>`，20s 心跳，10min 超时 |

#### 对系统的帮助
- 外部客户端（Cursor、Claude Desktop、n8n、Dify 等）可通过标准 MCP SSE 协议接入小云全部 21 个 AgentTool
- AppKey 签名鉴权与现有 `TenantAppOrchestrator.authenticateByAppKey()` 复用，无额外安全风险
- `?token=<jwt>` URL 参数方式无需额外改动 TokenAuthFilter（本已支持）
- `SecurityConfig` 仅放行 `/api/intelligence/mcp/token`，SSE 和 messages 端点仍需有效 JWT

#### 顺带修复
- `ProductionOrderProgressRecomputeService.java` 导入错误（`service.impl.ProductWarehousingService` → `service.ProductWarehousingService`）

#### Cursor / Claude Desktop 接入示例
```bash
# 第一步：换取 MCP Token
curl -X POST https://your-domain/api/intelligence/mcp/token \
  -H 'X-App-Key: <appKey>' \
  -H 'X-Timestamp: 1714800000' \
  -H 'X-Signature: <hmac-sha256(timestamp)>'

# 第二步：配置 ~/.cursor/mcp.json
{
  "mcpServers": {
    "小云": {
      "url": "https://your-domain/api/intelligence/mcp/sse?token=<mcpToken>"
    }
  }
}
```
...
## 🧭 工序节点完成时间兜底修复 — 尾部已完成但节点时间显示 `--`

### fix(frontend): 已完成节点优先回退后端阶段时间字段，避免 `boardTimesByOrder` 未命中时误显示空白

#### 问题现象
- 订单管理 / 工序跟进页里，部分订单的 `尾部` 节点已经显示 100%，
  但节点下方完成时间仍是 `--`。
- 直接查订单列表接口可以看到，后端其实已经返回了：
  - `tailProcessRate = 100`
  - `packagingEndTime` / `ironingEndTime` 有真实值

#### 根本原因
- 前端节点时间主要依赖 `boardTimesByOrder`（扫码记录二次聚合）推导；
  `尾部`、`车缝`、`二次工艺` 这些阶段即便后端接口已经返回了完成时间字段，
  之前也没有作为展示兜底使用。
- 于是只要时间桶某次未命中节点 key，就会出现：
  - 完成率来自后端 `*CompletionRate`
  - 节点时间却因为前端聚合未命中而显示 `--`

#### 修复内容
- 新增共享 helper：
  `frontend/src/modules/production/pages/Production/ProgressDetail/utils.ts`
  - `getOrderStageCompletionTimeFallback()`
- 两处节点时间展示统一接入后端字段兜底：
  - `frontend/src/modules/production/pages/Production/List/hooks/useProgressTracking.tsx`
  - `frontend/src/modules/production/pages/Production/ProgressDetail/hooks/useProgressColumns.tsx`
...
#### 对系统的帮助
- ✅ 修复“尾部已完成但节点时间空白”的显示断层。
- ✅ 前端即使某次 `boardTimesByOrder` 未命中，也能直接消费后端已有阶段时间。
- ✅ 避免用户误以为“后端没有传时间”，实际是前端没吃兜底字段。
...
## 🤖 AI Agent Tool 扩容 + 多租户安全收口 + 定时任务观测修复

### feat(intelligence): 新增 13 个 AgentTool，补齐财务 / 电商 / 仓库 / 样衣改版 / 数据字典能力

#### 新增工具文件

- `InvoiceTool.java`（`backend/.../InvoiceTool.java`）
  - `tool_invoice`：发票列表 / 详情 / 统计 / 创建 / 开票 / 作废
- `FinancialReportTool.java`（`backend/.../FinancialReportTool.java`）
  - `tool_financial_report`：利润表 / 资产负债表 / 现金流量表
- `EcSalesRevenueTool.java`（`backend/.../EcSalesRevenueTool.java`）
  - `tool_ec_sales_revenue`：电商营收记录与汇总统计
- `TaxConfigTool.java`（`backend/.../TaxConfigTool.java`）
  - `tool_tax_config`：税率列表 / 当前生效税率 / 税费计算
- `EcommerceOrderTool.java`（`backend/.../EcommerceOrderTool.java`）
  - `tool_ecommerce_order`：电商订单列表 / 详情 / 统计
- `OrderTransferTool.java`（`backend/.../OrderTransferTool.java`）
  - `tool_order_transfer`：订单转单列表 / 创建 / 接受 / 拒绝
- `StyleQuotationTool.java`（`backend/.../StyleQuotationTool.java`）
  - `tool_style_quotation`：款式报价查询 / 创建 / 审核 / 解锁
- `PatternRevisionTool.java`（`backend/.../PatternRevisionTool.java`）
  - `tool_pattern_revision`：样衣改版记录 / 提交审批 / 审批 / 驳回
- `MaterialRollTool.java`（`backend/.../MaterialRollTool.java`）
  - `tool_material_roll`：物料卷列表 / 详情 / 扫码
- `MaterialQualityIssueTool.java`
  （`backend/.../MaterialQualityIssueTool.java`）
  - `tool_material_quality_issue`：物料质量问题列表 / 统计 / 解决
- `InventoryCheckTool.java`（`backend/.../InventoryCheckTool.java`）
  - `tool_inventory_check`：盘点列表 / 详情 / 统计 / 创建 / 确认 / 取消
- `SupplierTool.java`（`backend/.../SupplierTool.java`）
  - `tool_supplier`：供应商（复用工厂实体）列表 / 详情
- `DictTool.java`（`backend/.../DictTool.java`）
  - `tool_dict`：数据字典列表 / 按类型查询

#### 同步改动

- `AiAgentToolAccessService.java`
  （`backend/.../AiAgentToolAccessService.java`）
  - 注册 13 个新工具；补充高风险工具名单；新增工厂账号域级 /
    工具级屏蔽规则
- `AiAgentDomainRouter.java`
  （`backend/.../AiAgentDomainRouter.java`）
  - 扩充电商订单、发票、税务、供应商、样衣改版、报价等关键词路由
- `helpers.ts`
  （`frontend/src/components/common/GlobalAiAssistant/helpers.ts`）
  - 新增工具中文展示名映射，保证小云面板展示不出现生硬英文工具名

#### 安全与统计口径修正

- 多个既有 AgentTool 的查询从“`tenantId != null` 才拼条件”
  收紧为显式租户过滤，避免租户上下文异常时放大查询范围。
- 多个涉及详情读取的工具从 `getById()` 改为
  `lambdaQuery().eq(id).eq(tenantId)`，把“查出来再判断”
  收口成“数据库层先隔离”。
- `DeepAnalysisTool`、`SmartReportTool`、`ProductionProgressTool`、
  `SystemOverviewTool`、`OrderComparisonTool` 等统计逻辑统一排除
  `scan_type = orchestration`，避免 AI/编排生成的系统记录污染
  真实扫码统计。
- `SecondaryProcessTool` 修复了二次工序创建时漏写 `tenantId` 的问题，并兼容现有测试/历史数据的租户校验逻辑。

### fix(intelligence): 定时任务观测切面不再把 scheduler 线程误判为“缺少租户上下文”

#### 相关文件

- `JobRunObservabilityAspect.java`
  （`backend/.../intelligence/aspect/JobRunObservabilityAspect.java`）
  - 移除 `TenantAssert.assertTenantContext()` 强制断言，允许平台级
    定时任务以 `tenantId = null` 写入任务执行日志
  - `logSuccess()` / `logFailed()` 外层再包一层 `try-catch`，避免
    审计日志写失败反过来打断真实定时任务
- `ScheduledConfig.java`（`backend/.../config/ScheduledConfig.java`）
  - 新增统一 `TaskScheduler` 配置：线程池大小 3，附带自定义
    `ErrorHandler`，把后续调度异常收口到结构化日志

#### 线上现象

- 云端日志持续每 2 秒刷：
  `Unexpected error occurred in scheduled task`
- 实际触发点是 `AsyncIntelligenceAuditService.flush()`（`fixedDelay = 2000`）。

#### 根因拆解

- `JobRunObservabilityAspect` 拦截所有 `@Scheduled` 方法。
- scheduler 线程默认没有登录用户/租户上下文。
- 切面在 `pjp.proceed()` 之前调用
  `TenantAssert.assertTenantContext()`，导致任务逻辑还没开始就被
  外层 Spring 定时器拦截并刷日志。

#### 修复效果

- 平台级 / 全局定时任务可正常执行并记录到 `t_ai_job_run_log`。
- 不再把 scheduler 线程错误当成普通请求线程处理。
- 云端每 2 秒一次的 `TaskUtils$LoggingErrorHandler` 日志风暴停止。
- 后续若调度逻辑内部再抛异常，也会先经过统一 `TaskScheduler`
  的 `ErrorHandler` 记录诊断日志。
- 即便任务运行审计写库失败，也只记 `warn`，不再影响实际任务执行。

#### 验证结果

- 前端类型检查通过：`cd frontend && npx tsc --noEmit`
- 后端全量测试通过：`Tests run: 898, Failures: 0, Errors: 0, Skipped: 1`

---

# 2026-04-23

## 📦 成品库存入库记录弹窗修复 — `/production/warehousing/list` 500 → 恢复正常返回

### fix(warehouse): ProductWarehousingServiceImpl 移除错误的 `exist=false` 临时字段查询

#### 问题现象
- 成品库存页点击「入库记录」时，浏览器请求
  `GET /api/production/warehousing/list?styleNo=HHY006&page=1&pageSize=500`
  直接返回 500。
- 弹窗顶部仍显示 `当前可用库存 372 件`，但列表区域是 `暂无数据`，
  底部汇总也变成 `总入库次数 0 / 累计入库 0`，造成
  “库存有、入库记录没了”的错觉。

#### 根本原因
- `ProductWarehousingServiceImpl.queryPage()` 为了规避 schema 漂移，
  已经切换成最小字段 `select(...)` 查询。
- 但同一段 `select(...)` 中误加入了 `ProductWarehousing::getColor` 和 `ProductWarehousing::getSize`。
- 这两个字段在实体上是 `@TableField(exist = false)` 的临时展示字段，
  并不属于 `t_product_warehousing` 的 MyBatis-Plus lambda 缓存列。
- 结果接口在组装查询阶段直接抛出：
  `MybatisPlusException: can not find lambda cache for this property [color]`

#### 修复内容
- 文件：`backend/src/main/java/com/fashion/supplychain/production/service/impl/ProductWarehousingServiceImpl.java`
- 从 `queryPage()` 的最小字段查询中移除：
  - `ProductWarehousing::getColor`
  - `ProductWarehousing::getSize`
- 保留后续在 `ProductWarehousingQueryHelper.list()` 中通过 `CuttingBundle` 回填颜色、尺码、裁剪数的既有逻辑，不改变前端展示结构。

#### 验证结果
- 本地重新编译并重启后端后，`/api/production/warehousing/list` 已从 500 恢复为 200。
- 使用同租户账号 `lilb / 123456` 验证：
  - `POST /api/warehouse/finished-inventory/list` 可返回 `HHY006`
    库存条目，`totalInboundQty = 372`
  - `GET /api/production/warehousing/list?styleNo=HHY006&page=1&pageSize=20`
    返回 `total = 30`、首页 `records = 20`
  - 首条记录可正确返回 `orderNo=PO20260401001`、`warehouse=A-001`、`color=蓝色`、`size=L`

#### 本次收益
- ✅ 修复成品库存「入库记录」弹窗 500，用户终于不会再看到“372 件库存却 0 条入库”的魔幻现场。
- ✅ 保留最小字段查询策略，继续降低 `t_product_warehousing` 扩展列变动对热点接口的脆弱性。
- ✅ 再次明确约束：`@TableField(exist = false)` 临时字段只能在查询后补充，
  不能放进 MyBatis-Plus lambda `select(...)`。
